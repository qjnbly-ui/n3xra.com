import { getAdminSession, type AdminSessionContext } from "/account/admin/admin-session.js";
...
interface WorkspaceChoice extends Row {
  id: string;
  organization_id: string;
  program_name: string;
  sender_name: string;
  status: string;
  organization: { id: string; name: string; account_status: string } | null;
}
...
const sections: Array<{ key: SectionKey; label: string; href: string; group: "workspace" | "readiness" }> = [
...
const pageDetails: Record<SectionKey, { title: string; description: string }> = {
...
function badge(value: unknown, tone = "neutral"): string {
...
function fact(labelText: string, value: unknown): string {
...
async function api<T>(scope: string, workspaceId = ""): Promise<T> {
  ...
  if (!response.ok) throw new Error(payload.error || "Communications Admin could not be loaded.");
  return payload;
}
...
function readiness(data: WorkspacePayload, channelName: "email" | "sms"): { label: string; tone: string; detail: string } {
  const channel = data.channels.find((row) => row.channel === channelName);
  if (channelName === "email") {
    const domain = data.sending_domains[0];
    if (!domain) return { label: "Not configured", tone: "neutral", detail: "No sending domain is connected." };
    if (domain.status === "verified" && channel?.status === "active") return { label: "Ready", tone: "ready", detail: `${domain.domain} is verified and the email channel is active.` };
    if (["pending", "pending_verification"].includes(domain.status) || channel?.status === "pending_verification") return { label: "Pending verification", tone: "pending", detail: `${domain.domain} is awaiting provider verification.` };
  ...
  if (!number) return { label: "Not configured", tone: "neutral", detail: "No Communications number is assigned." };
  ...
  if (number.status === "active" && number.texting_activated_at && channel?.status === "active") return { label: "Ready", tone: "ready", detail: `${number.phone_e164} is registered and active.` };
  return { label: label(channel?.status || number.status), tone: "pending", detail: `${number.phone_e164} is not activated for sending.` };
}

function renderContext(workspaces: WorkspaceChoice[]): void {
...
function renderWebsitesForms(data: WorkspacePayload): string {
...
function renderSubscribers(data: WorkspacePayload): string {
  ...
  const topicsBySubscriber = new Map<string, string[]>();
  data.subscriber_topics.forEach((choice) => {
    ...
    if (topicName) topicsBySubscriber.set(choice.subscriber_id, [...current, topicName]);
  });
...
function renderTopicsSignup(data: WorkspacePayload): string {
...
function renderActivityUsage(data: WorkspacePayload): string {
  ...
  return `<section class="communications-admin-metrics">${fact("Plan", label(data.workspace.plan_key))}${fact("SMS segments", Number(metrics.sms_segments_current_month || 0).toLocaleString())}${fact("SMS allowance", Number(data.workspace.included_sms_segments || 0).toLocaleString())}${fact("Email deliveries", Number(metrics.email_deliveries_current_month || 0).toLocaleString())}${fact("Email allowance", Number(data.workspace.included_email_deliveries || 0).toLocaleString())}${fact("Outbound MMS", Number(metrics.outbound_mms_current_month || 0).toLocaleString())}${fact("Consent events", Number(metrics.consent_events || data.consent_events.length))}${fact("Message events", Number(metrics.message_events || data.message_events.length))}${fact("Form submissions", data.submissions.length)}${fact("Queued actions", data.queue.length)}</section><div class="communications-admin-grid">${card("Administrative audit", "Immutable records created by trusted Communications Admin operations.", `<div class="communications-admin-table-wrap"><table><thead><tr><th>Time</th><th>Action</th><th>Record</th><th>Administrator</th></tr></thead><tbody>${audit}</tbody></table></div>`)}${card("Message activity", "Delivery and inbound/outbound event records. No composer or send controls are present.", `<div class="communications-admin-table-wrap"><table><thead><tr><th>Time</th><th>Channel</th><th>Direction</th><th>Status</th><th>Preview</th><th>Units</th></tr></thead><tbody>${messages}</tbody></table></div>`)}${card("Form action queue", "Read-only processing state for universal form actions.", `<div class="communications-admin-table-wrap"><table><thead><tr><th>Created</th><th>Status</th><th>Attempts</th><th>Last error</th></tr></thead><tbody>${queue}</tbody></table></div>`)}</div>`;
}
...
function renderTextingReadiness(data: WorkspacePayload): string {
  ...
  const onboardingBody = onboarding ? `
    ...
    <section><strong>Sample messages</strong><div class="communications-admin-list">${Array.isArray(application.message_samples) ? application.message_samples.map((message: unknown, index: number) => `<article class="communications-admin-list-row"><div><strong>Sample ${index + 1}</strong><small>${escapeHtml(message)}</small></div></article>`).join("") : empty("No sample messages were provided.")}</div></section>
...
function renderWorkspaceSection(data: WorkspacePayload): string {
  switch (section) {
...
function setLoading(message: string): void {
  const content = root?.querySelector<HTMLElement>("#communications-admin-content");
  if (content) content.innerHTML = `<div class="communications-admin-loading"><span></span><p>${escapeHtml(message)}</p></div>`;
}
