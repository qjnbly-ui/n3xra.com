...
let supabase: any;
let escapeHtml: Dependencies["escapeHtml"];
let formatDate: Dependencies["formatDate"];
let setStatus: Dependencies["setStatus"];
let confirmAdminAction: Dependencies["confirmAdminAction"];
let reviews: Row[] = [];
let subjects = new Map<string, Row>();
let selectedReviewId = "";

const element = <T extends HTMLElement>(id: string): T => {
  const found = document.getElementById(id);
  if (!found) throw new Error(`Customer Reviews control is missing: ${id}`);
  return found as T;
};

const statusLabel = (status: string): string => ({ pending: "Awaiting review", changes_requested: "Changes requested", published: "Published", hidden: "Hidden", rejected: "Not approved" }[status] || status);
const scopeLabel = (scope: string): string => scope === "organization" ? "Official organization review" : "Personal review";
const reviewerLabel = (review: Row): string => review.scope === "organization" ? review.organization_name_snapshot || "Organization" : review.reviewer_name_snapshot || "Customer";
const subjectLabel = (review: Row): string => subjects.get(review.subject_key)?.name || String(review.subject_key || "").replaceAll("_", " ");

function filteredReviews(): Row[] {
  const query = element<HTMLInputElement>("reviews-admin-search").value.trim().toLowerCase();
  const status = element<HTMLSelectElement>("reviews-admin-status-filter").value;
  const scope = element<HTMLSelectElement>("reviews-admin-scope-filter").value;
  return reviews.filter((review) => {
    if (status !== "all" && review.status !== status) return false;
    if (scope !== "all" && review.scope !== scope) return false;
...
function renderDetail(): void {
  ...
  if (!review) { detail.innerHTML = '<div class="review-list-empty"><strong>Select a review</strong><p>Choose a submission to read it and make a moderation decision.</p></div>'; return; }
...
function render(): void { renderMetrics(); renderList(); renderDetail(); }
...
async function loadReviews(): Promise<void> {
  ...
  if (subjectResult.error || reviewResult.error) throw subjectResult.error || reviewResult.error;
  subjects = new Map((subjectResult.data || []).map((subject: Row) => [subject.subject_key, subject]));
  reviews = reviewResult.data || [];
  if (selectedReviewId && !reviews.some((review) => review.id === selectedReviewId)) selectedReviewId = "";
  render();
}

async function moderate(status: string): Promise<void> {
  const review = reviews.find((item) => item.id === selectedReviewId);
  if (!review) return;
  const note = element<HTMLTextAreaElement>("reviews-admin-note").value.trim();
  if (status === "changes_requested" && note.length < 3) { element("reviews-admin-detail-status").textContent = "Add a short note explaining what should change."; return; }
  ...
  if (error) { element("reviews-admin-detail-status").textContent = error.message; return; }
  await loadReviews();
  setStatus(status === "published" ? "Review published." : "Moderation decision saved.", "success");
}

async function setFeatured(isFeatured: boolean): Promise<void> {
  const review = reviews.find((item) => item.id === selectedReviewId);
...
function bindEvents(): void {
  ...
  element("reviews-admin-list").addEventListener("click", (event) => { const row = (event.target as HTMLElement).closest<HTMLButtonElement>("[data-review-id]"); if (!row) return; selectedReviewId = row.dataset.reviewId || ""; renderList(); renderDetail(); });
  element("reviews-admin-detail").addEventListener("click", (event) => {
    const target = event.target as HTMLElement;
    const statusButton = target.closest<HTMLButtonElement>("[data-review-status]");
    if (statusButton?.dataset.reviewStatus) { void moderate(statusButton.dataset.reviewStatus); return; }
    const featuredButton = target.closest<HTMLButtonElement>("[data-review-featured]");
    if (featuredButton?.dataset.reviewFeatured) void setFeatured(featuredButton.dataset.reviewFeatured === "true");
  });
}

export async function startReviews(context: Dependencies): Promise<void> {
  supabase = context.supabase; escapeHtml = context.escapeHtml; formatDate = context.formatDate; setStatus = context.setStatus; confirmAdminAction = context.confirmAdminAction;
  bindEvents();
  await loadReviews();
}
