...
type ReviewSubject = { subject_key: string; name: string; personal_reviews_enabled: boolean };
type PersonalReview = { id: string; subject_key: string; rating: number; review_text: string; status: string; moderation_note: string | null; updated_at: string };

const element = <T extends HTMLElement>(id: string): T => {
  const found = document.getElementById(id);
  if (!found) throw new Error(`Review control is missing: ${id}`);
  return found as T;
};
...
let session: any;
let subjects: ReviewSubject[] = [];
let reviews: PersonalReview[] = [];

function selectedReview(): PersonalReview | null {
  const key = element<HTMLSelectElement>("personal-review-subject").value;
...
function setMessage(copy = "", tone = ""): void {
...
function renderEditor(): void {
  ...
  element<HTMLSelectElement>("personal-review-rating").value = String(review?.rating || 5);
  element<HTMLTextAreaElement>("personal-review-text").value = review?.review_text || "";
  ...
  element<HTMLButtonElement>("personal-review-delete").hidden = !review;
...
async function save(event: SubmitEvent): Promise<void> {
  ...
  const payload = { scope: "personal", subject_key: element<HTMLSelectElement>("personal-review-subject").value, author_user_id: session.user.id, organization_id: null, rating: Number(element<HTMLSelectElement>("personal-review-rating").value), review_text: element<HTMLTextAreaElement>("personal-review-text").value.trim(), reviewer_name_snapshot: "N3XRA customer", organization_name_snapshot: null, status: "pending" };
  ...
  if (result.error) { setMessage(result.error.message, "error"); return; }
  await loadReviews(); renderEditor(); renderList(); setMessage("Submitted. N3XRA will review it before it is published.", "success");
}
...
async function remove(): Promise<void> {
  ...
  await loadReviews(); renderEditor(); renderList(); setMessage("Review deleted.", "success");
}

async function init(): Promise<void> {
  if (!hasConfig() || !supabase) throw new Error("Reviews are temporarily unavailable.");
  session = await getSessionOrNull(supabase);
  if (!session?.user) { window.location.replace("/account/?next=%2Faccount%2Freviews%2F"); return; }
  ...
  element<HTMLSelectElement>("personal-review-subject").innerHTML = subjects.map((subject) => `<option value="${escapeHtml(subject.subject_key)}">${escapeHtml(subject.name)}</option>`).join("");
  await loadReviews();
  element("personal-review-subject").addEventListener("change", () => { renderEditor(); renderList(); });
  element<HTMLFormElement>("personal-review-form").addEventListener("submit", (event) => void save(event));
  element("personal-review-delete").addEventListener("click", () => void remove());
  element("personal-review-list").addEventListener("click", (event) => { const id = (event.target as HTMLElement).closest<HTMLButtonElement>("[data-edit-review]")?.dataset.editReview; const review = reviews.find((item) => item.id === id); if (review) { element<HTMLSelectElement>("personal-review-subject").value = review.subject_key; renderEditor(); renderList(); } });
  ...
  renderEditor(); renderList(); document.body.classList.remove("reviews-loading-state"); element("reviews-loading").hidden = true;
}

void init().catch((error: unknown) => { document.body.classList.remove("reviews-loading-state"); element("reviews-loading").textContent = error instanceof Error ? error.message : "Reviews could not be opened."; });
