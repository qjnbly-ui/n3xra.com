...
type Rates = { apparatus: RateMap; personnel: RateMap };
...
function readRates(): Rates {
  try {
    const stored = JSON.parse(localStorage.getItem(RATES_KEY) || "null") as Rates | null;
...
function readInvoices(): Invoice[] {
  try {
    const stored = JSON.parse(localStorage.getItem(INVOICES_KEY) || "null") as Invoice[] | null;
    return Array.isArray(stored) ? stored : [sampleInvoice()];
  } catch { return [sampleInvoice()]; }
}

function writeInvoices(invoices: Invoice[]) { localStorage.setItem(INVOICES_KEY, JSON.stringify(invoices)); }
...
function ratesPage() {
  const rates = readRates();
  const rateRows = (map: RateMap, group: keyof Rates) => Object.entries(map).map(([name, rate]) => `<div class="rate-row"><strong>${escapeHtml(name)}</strong><label class="rate-field"><input type="number" min="0" step=".01" value="${rate}" data-rate-group="${group}" data-rate-name="${escapeHtml(name)}" aria-label="${escapeHtml(name)} hourly rate"></label></div>`).join("");
...
function bindEditor() {
  ...
  document.querySelector(".invoice-content")?.addEventListener("click", (event) => {
    ...
    if (button.id === "print-invoice") window.print();
  });
}
...
function bindRates() {
  document.querySelector("#save-rates")?.addEventListener("click", () => {
    ...
    document.querySelectorAll<HTMLInputElement>("[data-rate-group]").forEach((input) => {
      const group = input.dataset.rateGroup as keyof Rates;
      const name = input.dataset.rateName || "";
      if (name) next[group][name] = number(input.value);
    });
    localStorage.setItem(RATES_KEY, JSON.stringify(next));
    const status = document.querySelector<HTMLElement>("#rates-status"); if (status) status.textContent = "Rates saved for new invoices.";
  });
