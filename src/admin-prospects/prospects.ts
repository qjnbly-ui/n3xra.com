...
interface Dependencies {
  ...
  confirmAdminAction: (message: string, options?: Record<string, unknown>) => Promise<boolean>;
}
...
let confirmAdminAction: Dependencies["confirmAdminAction"];
let contacts: Row[] = [];
let currentContact: Row | null = null;
let pendingCardBlob: Blob | null = null;
let pendingCardDataUrl = "";
let pendingPreviewUrl = "";
let scanMetadata: Row = {};

const element = <T extends HTMLElement>(id: string): T => {
  const found = document.getElementById(id);
  if (!found) throw new Error(`Sales Leads control is missing: ${id}`);
  return found as T;
};

const input = (id: string): HTMLInputElement => element<HTMLInputElement>(id);
const select = (id: string): HTMLSelectElement => element<HTMLSelectElement>(id);
...
function normalizePhone(value: unknown): string | null {
  ...
  if (!/^[1-9][0-9]{7,14}$/.test(digits)) throw new Error("Enter a valid phone number with area code.");
  return `+${digits}`;
}
...
function setPageStatus(message = "", tone = ""): void {
  const status = element<HTMLElement>("admin-status");
  status.textContent = message;
  status.className = `admin-status${tone ? ` ${tone}` : ""}`;
}

function setDialogStatus(message = "", tone = ""): void {
  const status = element<HTMLElement>("prospect-form-status");
  status.textContent = message;
  status.className = `prospect-form-status${tone ? ` is-${tone}` : ""}`;
}

function setScanStatus(message = "", tone = ""): void {
  const status = element<HTMLElement>("prospect-scan-status");
  status.textContent = message;
...
function displayName(contact: Row): string {
...
function filteredContacts(): Row[] {
  ...
  return contacts.filter((contact) => {
    if (status !== "all" && contact.relationship_status !== status) return false;
    if (interest !== "all" && !(contact.interest_tags || []).includes(interest)) return false;
...
function renderInterestFilter(): void {
...
function renderContacts(): void {
  ...
  list.innerHTML = rows.length ? rows.map((contact) => {
    const tags = (contact.interest_tags || []).slice(0, 4).map((tag: string) => `<i>${escapeHtml(tag)}</i>`).join("");
...
function showCardPreview(url = ""): void {
  clearPreviewUrl();
...
function resetForm(): void {
  element<HTMLFormElement>("prospect-form").reset();
  ...
  scanMetadata = {};
  showCardPreview();
  element<HTMLButtonElement>("prospect-card-analyze").disabled = true;
  element<HTMLButtonElement>("prospect-delete").hidden = true;
  setScanStatus();
  setDialogStatus();
}

function populateForm(contact: Row): void {
  ...
  element<HTMLTextAreaElement>("prospect-notes").value = contact.notes || "";
...
async function openDialog(contact: Row | null = null, scanFirst = false): Promise<void> {
  resetForm();
  currentContact = contact;
  element("prospect-dialog-title").textContent = contact ? "Edit sales lead" : scanFirst ? "Scan a business card" : "Add a sales lead";
  element<HTMLButtonElement>("prospect-delete").hidden = !contact || document.body.dataset.adminRole === "sales_rep";
  if (contact) {
  ...
  element<HTMLDialogElement>("prospect-dialog").showModal();
  if (scanFirst) input("prospect-card-input").click();
  else input("prospect-full-name").focus();
}

function closeDialog(): void {
  element<HTMLDialogElement>("prospect-dialog").close();
  clearPreviewUrl();
}
...
async function compressCard(file: File): Promise<Blob> {
  ...
  if (file.size > 16_000_000) throw new Error("Choose a business-card image smaller than 16 MB.");
  ...
  for (const quality of [.88, .78, .68, .58]) {
    ...
    if (blob.size <= MAX_SCAN_BYTES) return blob;
  }
...
async function chooseCard(event: Event): Promise<void> {
  ...
  try {
    ...
    element<HTMLButtonElement>("prospect-card-analyze").disabled = false;
...
function applyScan(details: ScanDetails): void {
  ...
  element<HTMLTextAreaElement>("prospect-notes").value = details.notes;
}

async function analyzeCard(): Promise<void> {
  if (!pendingCardDataUrl || !session?.access_token) return;
  const button = element<HTMLButtonElement>("prospect-card-analyze");
  ...
  try {
    ...
    if (!response.ok) throw new Error(payload.error || "The card could not be analyzed.");
    applyScan(payload.details as ScanDetails);
...
function consentTimestamp(status: string, currentValue: unknown): string | null {
  if (status !== "subscribed") return currentValue ? String(currentValue) : null;
...
function formPayload(): Row {
  ...
  if (emailStatus === "subscribed" && !email) throw new Error("Add an email before enabling email messages.");
  if (smsStatus === "subscribed" && !phone) throw new Error("Add a phone number before enabling text messages.");
  if ((emailStatus === "subscribed" || smsStatus === "subscribed") && !consentNotes) throw new Error("Record how permission was received before enabling messages.");

  const fullName = clean(input("prospect-full-name").value, 180);
  const companyName = clean(input("prospect-company").value, 220);
  if (!fullName && !companyName && !email && !phone) throw new Error("Add a name, company, email, or phone number.");
  ...
  return {
    ...
    notes: clean(element<HTMLTextAreaElement>("prospect-notes").value, 4000),
...
async function uploadPendingCard(): Promise<string | null> {
  if (!pendingCardBlob) return currentContact?.card_image_path || null;
...
async function saveProspect(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  const button = element<HTMLButtonElement>("prospect-save");
  button.disabled = true;
  ...
  try {
    ...
    if (uploadedPath && uploadedPath !== currentContact?.card_image_path) await supabase.storage.from(BUCKET).remove([uploadedPath]);
    const message = error instanceof Error ? error.message : String((error as Row)?.message || "The sales lead could not be saved.");
...
async function deleteProspect(): Promise<void> {
  ...
  if (error) return setDialogStatus(error.message, "error");
  if (currentContact.card_image_path) await supabase.storage.from(BUCKET).remove([currentContact.card_image_path]);
  closeDialog();
  await loadContacts();
}
...
function bindEvents(): void {
  ...
  element<HTMLFormElement>("prospect-form").addEventListener("submit", (event) => void saveProspect(event as SubmitEvent));
  ...
  element("prospect-list").addEventListener("click", (event) => {
    const id = (event.target as HTMLElement).closest<HTMLElement>("[data-prospect-id]")?.dataset.prospectId;
    const contact = contacts.find((item) => item.id === id);
    if (contact) void openDialog(contact);
  });
  element<HTMLDialogElement>("prospect-dialog").addEventListener("click", (event) => {
    if (event.target === element("prospect-dialog")) closeDialog();
  });
}

export async function startProspects(dependencies: Dependencies): Promise<void> {
