...
export async function initializeRecordsPortalShell(): Promise<void> {
  ...
  const desktopBrand = document.querySelector<HTMLAnchorElement>(".records-desktop-app-brand");
  if (desktopBrand) {
    desktopBrand.href = "/client-portal/";
    desktopBrand.setAttribute("aria-label", `${identity.websiteName} portal home`);
    const image = desktopBrand.querySelector<HTMLImageElement>("img");
    if (image) {
    ...
    const name = desktopBrand.querySelector<HTMLElement>("span");
    if (name) name.textContent = identity.websiteName;
  }

  const mobileBrand = document.querySelector<HTMLAnchorElement>(".brand-home-link");
  if (mobileBrand) {
    mobileBrand.href = "/client-portal/";
    mobileBrand.setAttribute("aria-label", `${identity.websiteName} portal home`);
    const image = mobileBrand.querySelector<HTMLImageElement>("img");
    if (image) {
      image.hidden = !identity.logoUrl;
      image.alt = identity.logoUrl ? `${identity.websiteName} logo` : "";
      if (identity.logoUrl) image.src = identity.logoUrl;
  ...
  document.querySelectorAll<HTMLElement>(".records-desktop-nav-label").forEach((label) => {
    label.textContent = "Records";
  });

  const actions = document.querySelector<HTMLElement>(".records-desktop-app-actions");
