...
function setupComposer(
  ...
  const openEmailPreview = async (): Promise<void> => {
    if (!message.value.trim() || !subject.value.trim()) {
      ...
      if (!subject.value.trim()) subject.focus();
      else message.focus();
      return;
    }
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result.ok || typeof result.html !== "string") throw new Error(result.error || "The email preview could not be prepared.");
