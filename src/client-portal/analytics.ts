...
interface WebsiteRow { id: string; name: string; }
interface TrendRow { timestamp?: string; pageviews?: number; visitors?: number; }
interface BreakdownRow { requestPath?: string; referrerHostname?: string; country?: string; deviceType?: string; eventName?: string; visitors?: number; pageviews?: number; count?: number; }
interface AnalyticsReport {
  generatedAt: string;
  cached: boolean;
  website: { id: string; name: string; liveUrl: string };
  source: { provider: string; projectName: string };
  period: { days: number | null; since: string; until: string; label: string };
  totals: { visitors: number; pageviews: number; pagesPerVisitor: number; events: number };
  trend: TrendRow[];
  breakdowns: { pages?: BreakdownRow[]; referrers?: BreakdownRow[]; countries?: BreakdownRow[]; devices?: BreakdownRow[]; events?: BreakdownRow[] };
  availability: Record<string, string>;
  warnings: Array<{ section?: string; message?: string }>;
  trendGranularity?: "day" | "month";
  breakdownsPeriodLabel?: string;
  archive?: { status: string; startedOn?: string | null; lastSyncedAt?: string | null };
}

const websiteSelect = document.querySelector<HTMLSelectElement>("#analytics-website-select");
const rangeSelect = document.querySelector<HTMLSelectElement>("#analytics-range");
const refreshButton = document.querySelector<HTMLButtonElement>("#analytics-refresh");
...
const dashboard = document.querySelector<HTMLElement>("#analytics-dashboard");
const screen = document.querySelector<HTMLElement>("#portal-status");
let session: any;
let websites: WebsiteRow[] = [];
let selectedWebsite: WebsiteRow | undefined;

function element<T extends HTMLElement>(id: string): T {
  const found = document.getElementById(id);
  if (!found) throw new Error(`Missing analytics element: ${id}`);
  return found as T;
}
...
function saveWebsite(website: WebsiteRow): void {
  try {
...
function displayLabel(value: string, kind: string): string {
  ...
  if (!clean || clean.toLowerCase() === "null") return kind === "referrer" ? "Direct / none" : "Unknown";
  if (kind === "country" && /^[A-Z]{2}$/i.test(clean)) {
    try { return new Intl.DisplayNames(undefined, { type: "region" }).of(clean.toUpperCase()) || clean.toUpperCase(); } catch { return clean.toUpperCase(); }
  }
  if (kind === "page") return clean || "/";
  return clean.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
}

function showStatus(copy: string, isError = false): void {
...
function renderList(id: string, rows: BreakdownRow[] | undefined, key: keyof BreakdownRow, metric: "visitors" | "count", kind: string, emptyCopy: string): void {
  const container = element<HTMLElement>(id);
...
function renderChart(rows: TrendRow[]): void {
  const chart = element<HTMLElement>("analytics-chart");
  if (!rows.length) {
...
function renderAnalytics(data: AnalyticsReport): void {
  ...
  if (status && report) { status.hidden = true; report.hidden = false; }
}

async function loadAnalytics(force = false): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) throw new Error(String(data?.error || "Website analytics could not be loaded."));
    renderAnalytics(data as AnalyticsReport);
  } catch (error) {
...
async function init(): Promise<void> {
  ...
  websites = scopeWebsitesToPortalTenant((data || []) as WebsiteRow[], tenant);
  if (!websites.length) throw new Error("No website analytics workspace is available for this account.");
  ...
  if (websiteSelect) {
    ...
    websiteSelect.addEventListener("change", () => {
      ...
      if (selectedWebsite) saveWebsite(selectedWebsite);
      void loadAnalytics();
    });
  }
  if (selectedWebsite) saveWebsite(selectedWebsite);
