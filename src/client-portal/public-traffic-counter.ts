...
function target(root: HTMLElement, name: "value" | "label"): HTMLElement {
...
async function load(root: HTMLElement): Promise<void> {
  ...
  try {
    ...
    root.hidden = false;
    root.dispatchEvent(new CustomEvent<CounterPayload>("n3xra:traffic-counter", { bubbles: true, detail: payload }));
  } catch {
    hide(root);
  }
}

document.querySelectorAll<HTMLElement>("[data-n3xra-traffic-counter]").forEach((root) => { void load(root); });
