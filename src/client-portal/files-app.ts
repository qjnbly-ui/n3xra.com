...
interface WebsiteVersion { id: string; asset_id: string; storage_bucket: string; storage_path: string; original_filename: string; mime_type: string | null; size_bytes: number | null; status: string; created_at: string; updated_at: string }
interface WebsiteFile extends WebsiteVersion { source: "website"; website_id: string; website_name: string; category: string; display_name: string; shared_with_n3xra: true }
type LibraryFile = OrganizationFile | WebsiteFile;

const PRIVATE_BUCKET = "organization-files-private";
const WEBSITE_PRIVATE_BUCKET = "website-assets-private";
const MAX_UPLOAD_BYTES = 50 * 1024 * 1024;
const CATEGORY_LABELS: Record<string, string> = { image: "Images", logo: "Logo", document: "Documents", brand: "Brand assets", social: "Social", journal: "Journal", visitor_submission: "Visitor submissions", other: "Other files" };
const CATEGORY_KEYS = Object.fromEntries(Object.entries(CATEGORY_LABELS).map(([key, label]) => [label.toLowerCase(), key]));

let supabase: any;
let userId = "";
let memberships: Array<{ organization_id: string; role: string; organization: Organization }> = [];
let activeOrganization: Organization | null = null;
let folders: Folder[] = [];
let organizationFiles: OrganizationFile[] = [];
let websites: Website[] = [];
let websiteAssets: WebsiteAsset[] = [];
let websiteVersions: WebsiteVersion[] = [];
...
const bytes = (value: number | null): string => { const size = Number(value || 0); if (size < 1024) return `${size} B`; if (size < 1024 ** 2) return `${(size / 1024).toFixed(1)} KB`; if (size < 1024 ** 3) return `${(size / 1024 ** 2).toFixed(1)} MB`; return `${(size / 1024 ** 3).toFixed(1)} GB`; };
const date = (value: string): string => new Date(value).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });
const safeName = (value: string): string => value.normalize("NFKD").replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^-+|-+$/g, "") || "file";
const assetKey = (value: string): string => { const base = value.replace(/\.[^.]+$/, "").toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "") || "asset"; return /^[a-z]/.test(base) ? base : `file-${base}`; };
const folderLabel = (category: string): string => CATEGORY_LABELS[category] || category.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());

function normalizeMembership(row: Membership): { organization_id: string; role: string; organization: Organization } | null {
...
function websiteFiles(): WebsiteFile[] {
  ...
  return websiteVersions.flatMap((version) => {
    ...
    return asset && website ? [{ ...version, source: "website" as const, website_id: website.id, website_name: website.name, category: asset.category, display_name: version.original_filename, shared_with_n3xra: true as const }] : [];
  });
}

function allFiles(): LibraryFile[] { return [...organizationFiles, ...websiteFiles()]; }

function locationFiles(): LibraryFile[] {
  if (activeLocation === "all") return allFiles();
  if (activeLocation === "private") return organizationFiles.filter((file) => !file.folder_id);
  if (activeLocation.startsWith("folder:")) return organizationFiles.filter((file) => file.folder_id === activeLocation.slice(7));
...
function fileType(file: LibraryFile): string {
...
function renderTree(): void {
  const target = one<HTMLElement>("#files-folder-tree");
...
function locationName(): string {
  if (activeLocation === "all") return "All files";
...
function renderFiles(): void {
  const list = one<HTMLElement>("#files-list");
  const status = one<HTMLElement>("#files-status");
  const query = one<HTMLInputElement>("#files-search")?.value.trim().toLowerCase() || "";
  ...
  if (one<HTMLElement>("#files-title")) one<HTMLElement>("#files-title")!.textContent = title;
  if (one<HTMLElement>("#files-breadcrumb")) one<HTMLElement>("#files-breadcrumb")!.textContent = `Files / ${title}`;
  if (one<HTMLElement>("#files-summary")) one<HTMLElement>("#files-summary")!.textContent = `${visible.length} file${visible.length === 1 ? "" : "s"} · ${bytes(visible.reduce((total, file) => total + Number(file.size_bytes || 0), 0))}`;
...
async function loadLibrary(): Promise<void> {
  ...
  folders = (folderResult.data || []) as Folder[];
  organizationFiles = (fileResult.data || []).map((file: Omit<OrganizationFile, "source">) => ({ ...file, source: "private" as const }));
  websites = (websiteResult.data || []) as Website[];
  ...
  websiteAssets = (assetResult.data || []) as WebsiteAsset[];
  ...
  websiteVersions = (versionResult.data || []) as WebsiteVersion[];
  renderFiles();
}

async function signedFile(file: LibraryFile, download = false): Promise<string> {
  const bucket = file.storage_bucket;
  const path = file.storage_path;
  if (!bucket || !path) throw new Error("This file does not have a stored copy.");
  ...
  if (error || !data?.signedUrl) throw new Error(error?.message || "Unable to prepare this file.");
  return data.signedUrl;
}

async function previewFile(key: string): Promise<void> {
  const [source = "", id = ""] = key.split(":");
  const file = allFiles().find((row) => row.source === source && row.id === id);
  if (!file) return;
  const url = await signedFile(file);
  const dialog = one<HTMLDialogElement>("#files-preview-dialog");
  const body = one<HTMLElement>("#files-preview-body");
  if (!dialog || !body) return;
  one<HTMLElement>("#files-preview-title")!.textContent = file.display_name;
  one<HTMLElement>("#files-preview-kicker")!.textContent = file.source === "website" ? "WEBSITE ASSET · SHARED WITH N3XRA" : file.shared_with_n3xra ? "SHARED WITH N3XRA" : "PRIVATE FILE";
  const download = one<HTMLAnchorElement>("#files-preview-download");
  if (download) { download.href = await signedFile(file, true); download.download = file.original_filename; }
  const mime = String(file.mime_type || "").toLowerCase();
...
async function uploadPrivateFile(file: File): Promise<void> {
  if (!activeOrganization) return;
...
async function uploadWebsiteFile(file: File, websiteId: string, category: string): Promise<void> {
  if (file.size > MAX_UPLOAD_BYTES) throw new Error(`${file.name} is larger than 50 MB.`);
  ...
  while (used.has(key)) key = `${assetKey(file.name)}-${suffix++}`;
...
async function start(): Promise<void> {
  ...
  memberships = ((data || []) as Membership[]).map(normalizeMembership).filter((row): row is { organization_id: string; role: string; organization: Organization } => Boolean(row));
  if (!memberships.length) throw new Error("Files & Assets needs an organization workspace.");
  ...
  const select = one<HTMLSelectElement>("#files-organization");
  if (select) { select.innerHTML = memberships.map((membership) => `<option value="${escape(membership.organization_id)}">${escape(membership.organization.name)}</option>`).join(""); select.value = activeOrganization.id; select.disabled = memberships.length === 1; }
  await loadLibrary();
  document.body.classList.remove("files-loading");
}

one("#files-folder-tree")?.addEventListener("click", (event) => { const button = (event.target as HTMLElement).closest<HTMLButtonElement>("[data-location]"); if (!button) return; activeLocation = button.dataset.location || "all"; renderFiles(); });
one("#files-search")?.addEventListener("input", renderFiles);
one("#files-organization")?.addEventListener("change", (event) => { const id = (event.target as HTMLSelectElement).value; activeOrganization = memberships.find((membership) => membership.organization_id === id)?.organization || null; if (!activeOrganization) return; setStoredActiveOrganizationId(id); activeLocation = "all"; void loadLibrary().catch(showError); });
one("#files-new-folder")?.addEventListener("click", () => one<HTMLDialogElement>("#files-folder-dialog")?.showModal());
document.querySelectorAll("[data-folder-close]").forEach((button) => button.addEventListener("click", () => one<HTMLDialogElement>("#files-folder-dialog")?.close()));
document.querySelectorAll("[data-preview-close]").forEach((button) => button.addEventListener("click", () => one<HTMLDialogElement>("#files-preview-dialog")?.close()));
one("#files-folder-form")?.addEventListener("submit", (event) => { event.preventDefault(); void (async () => { if (!activeOrganization) return; const form = event.currentTarget as HTMLFormElement; const name = String(new FormData(form).get("name") || "").trim(); const { error } = await supabase.from("organization_file_folders").insert({ organization_id: activeOrganization.id, parent_id: null, name, source_product: "files_assets", shared_with_n3xra: false, is_system: false, created_by_user_id: userId }); if (error) throw error; form.reset(); one<HTMLDialogElement>("#files-folder-dialog")?.close(); await loadLibrary(); })().catch(showError); });
one("#files-upload")?.addEventListener("change", (event) => { const input = event.target as HTMLInputElement; const selected = [...(input.files || [])]; input.value = ""; void (async () => { const websiteParts = activeLocation.startsWith("website:") ? activeLocation.split(":") : []; const websiteId = websiteParts[1] || ""; const category = websiteParts[2] || "other"; for (const file of selected) { showStatus(`Uploading ${file.name}…`); if (websiteId) await uploadWebsiteFile(file, websiteId, CATEGORY_KEYS[folderLabel(category).toLowerCase()] || category); else await uploadPrivateFile(file); } await loadLibrary(); })().catch(showError); });
one("#files-list")?.addEventListener("click", (event) => { const target = (event.target as HTMLElement).closest<HTMLElement>("[data-open],[data-share],[data-delete]"); if (!target) return; if (target.dataset.open) void previewFile(target.dataset.open).catch(showError); if (target.dataset.share) void (async () => { const file = organizationFiles.find((row) => row.id === target.dataset.share); if (!file) return; const next = !file.shared_with_n3xra; if (next && !window.confirm("Share this file with N3XRA support? N3XRA administrators will be able to open and download it until you stop sharing.")) return; const { error } = await supabase.from("organization_files").update({ shared_with_n3xra: next }).eq("id", file.id); if (error) throw error; await loadLibrary(); })().catch(showError); if (target.dataset.delete) void (async () => { const file = organizationFiles.find((row) => row.id === target.dataset.delete); if (!file || !window.confirm(`Delete “${file.display_name}”? This cannot be undone.`)) return; const { error } = await supabase.from("organization_files").delete().eq("id", file.id); if (error) throw error; if (file.storage_bucket && file.storage_path) await supabase.storage.from(file.storage_bucket).remove([file.storage_path]); await loadLibrary(); })().catch(showError); });

function showStatus(message: string): void { const status = one<HTMLElement>("#files-status"); if (status) { status.hidden = false; status.textContent = message; } }
function showError(error: unknown): void { showStatus(error instanceof Error ? error.message : "Unable to update Files & Assets."); }
