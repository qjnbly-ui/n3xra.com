...
interface CatalogProduct { product_key: string; name: string; portal_path: string; }
interface Enrollment { product_key: string; status: string; portal_enabled: boolean; product: CatalogProduct | CatalogProduct[] | null; }
interface Workspace { id: string; slug: string; }
const html = (value: unknown): string => String(value ?? "").replace(/[&<>"']/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]!));
export function organizationProductLink(path: string, organizationId: string): string {
...
export async function startOrganizations({ supabase }: { supabase: any }): Promise<void> {
  ...
  const card = (name: string, workspace: string, status: string, preview: string, admin = "", adminLabel = "Open admin workspace"): string => `<article class="account-access-card"><div><span>${html(name)}</span><h4>${html(workspace)}</h4><p>${html(status)}</p></div><div class="account-admin-head-actions">${preview ? `<a class="portal-button portal-button-secondary" href="${html(preview)}">Preview client view</a>` : ""}${admin ? `<a class="portal-button portal-button-secondary" href="${html(admin)}">${html(adminLabel)}</a>` : ""}</div></article>`;
  async function show(id: string): Promise<void> {
    const org = organizations.find(o => o.id === id);
    if (!org) { detail.textContent = "Choose an organization to view its products and members."; return; }
    ...
    const [websitesResult, productsResult, enrollmentResult, teamPage] = await Promise.all([
      ...
      fetch("/client-portal/team/").then(response => { if (!response.ok) throw new Error("Organization Admin could not be loaded."); return response.text(); }),
    ]);
    if (request !== revision || !root!.isConnected) return;
    const failed = [websitesResult, productsResult, enrollmentResult].find(r => r.error); if (failed) throw failed.error;
    const websites: Website[] = websitesResult.data || [];
    const products: PrivateProduct[] = productsResult.data || [];
    const enrollments: Enrollment[] = enrollmentResult.data || [];
    ...
    for (const enrollment of enrollments) {
      if (enrollment.product_key === "website") continue;
      ...
      let admin = "";
      if (product.product_key === "records") { preview = `/n3xra-records/library/?support_org=${encodeURIComponent(id)}`; admin = `/n3xra-admin/records/organizations/?organization=${encodeURIComponent(id)}`; }
      if (product.product_key === "project_cards") admin = `/n3xra-admin/project-cards/?user=${encodeURIComponent(org.owner_user_id)}&organization=${encodeURIComponent(id)}`;
      if (product.product_key === "communications") {
        ...
        const workspace: Workspace | undefined = result.data?.[0];
        if (workspace && result.data.length === 1) { preview = `/client-portal/communications/?workspace=${encodeURIComponent(workspace.slug)}`; admin = `/n3xra-admin/communications/?workspace=${encodeURIComponent(workspace.id)}`; }
      }
    ...
    for (const product of products) cards.push(card(product.name,org.name,`Private · ${product.status}`,product.status === "active" ? privateProductPath(product.app_path,id,product.id) : ""));
    if (request !== revision || !root!.isConnected) return;
    ...
    if (!teamElement) throw new Error("Organization Admin layout is unavailable.");
    const host = detail.querySelector<HTMLElement>("#org-team-host")!;
