...
interface WebsiteProject {
  ...
  managed_website_id: string | null;
}
...
function dismissalKey(userId: string, proposalId: string): string {
...
function reviewUrl(proposalId: string): string {
...
async function pendingProposalsForCurrentPortal(supabase: any): Promise<PendingProposal[]> {
  ...
  const projects = (projectResult.data || []) as WebsiteProject[];
  let proposals = (proposalResult.data || []) as PendingProposal[];
  ...
  const versionIds = proposals.map((proposal) => proposal.current_version_id).filter((id): id is string => Boolean(id));
  ...
  const eligibleVersionByProposal = new Map(
    ((versionResult.data || []) as ProposalVersion[])
...
function markDismissed(userId: string, proposalId: string): void {
  try {
...
function wasDismissed(userId: string, proposalId: string): boolean {
  try {
...
function renderNotice(proposals: PendingProposal[], userId: string): void {
  const proposal = proposals[0];
  if (!proposal || document.querySelector("[data-pending-proposal-notice]")) return;
  ...
  const dismiss = (): void => markDismissed(userId, proposal.id);
  ...
  window.addEventListener("n3xra:proposal-resolved", (event) => {
    const resolvedId = event instanceof CustomEvent && typeof event.detail?.proposalId === "string"
      ...
      : "";
    if (resolvedId !== proposal.id) return;
...
export function clearPendingProposalNoticeDismissals(): void {
  try {
...
export async function initializePendingProposalNotice(): Promise<void> {
  if (!hasConfig()) return;
