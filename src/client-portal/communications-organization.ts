...
function clientWorkspaceWebsiteId(userId: string): string {
  const tenantWebsiteId = String(document.body.dataset.portalWebsiteId || "").trim();
  if (UUID_PATTERN.test(tenantWebsiteId)) return tenantWebsiteId;
  try {
...
async function organizationForWebsite(supabase: any, websiteId: string): Promise<string> {
  if (!websiteId) return "";
...
async function hasActiveEntitlement(supabase: any, organizationId: string): Promise<boolean> {
...
async function canAdministerOrganization(supabase: any, organizationId: string, userId: string): Promise<boolean> {
...
async function eligibleOrganization(supabase: any, organizationId: string, userId: string, options: ResolutionOptions): Promise<string> {
  if (!await hasActiveEntitlement(supabase, organizationId)) return "";
  if (options.requireAccountAdmin && !await canAdministerOrganization(supabase, organizationId, userId)) return "";
  return organizationId;
}

export async function resolveSelectedCommunicationsOrganization(
  supabase: any,
  userId: string,
  ...
  const { data, error } = await supabase.from("organization_product_entitlements")
    ...
    .limit(2);
  if (error) throw error;
  const organizationIds = [...new Set<string>((data || []).map((row: { organization_id?: string }) => String(row.organization_id || "")).filter(Boolean))];
