...
interface Account { id: string; email: string; name: string; emailConfirmedAt?: string | null; access?: Array<Record<string, unknown>> }
interface Organization { id: string; name: string; account_status: string; owner_user_id: string }
interface Membership { organization_id: string; user_id: string; role: string }
...
function assignmentsFor(accountId: string): Array<{ organization: Organization; role: string }> {
  ...
  memberships.filter((membership) => membership.user_id === accountId).forEach((membership) => {
    ...
    if (organization) result.set(organization.id, { organization, role: membership.role });
  });
...
function renderAccountList(): void {
  ...
  const target = one<HTMLElement>("#pca-account-list");
  if (target) target.innerHTML = visible.length ? visible.map((account) => {
  ...
  one<HTMLElement>("#pca-account-count")!.textContent = String(accounts.length);
}

function assignmentRow(account: Account, assignment: { organization: Organization; role: string }): string {
...
function renderDetail(): void {
  ...
  if (!account) { if (empty) empty.hidden = false; if (detail) detail.hidden = true; return; }
  ...
  one<HTMLElement>("#pca-detail-title")!.textContent = account.name || account.email;
  one<HTMLElement>("#pca-detail-summary")!.textContent = `${account.email} · Project Cards is available whether or not this account has a client website.`;
  one<HTMLElement>("#pca-organization-total")!.textContent = String(assignments.length);
  one<HTMLElement>("#pca-workspace-total")!.textContent = String(activeAssignments.length);
  one<HTMLElement>("#pca-content-total")!.textContent = `${accountProjects.length} / ${accountCards.length}`;
  const accountLink = one<HTMLAnchorElement>("#pca-account-link");
  if (accountLink) accountLink.href = `/account/admin/accounts/?${new URLSearchParams({ user: account.id, email: account.email }).toString()}`;
  const assignmentList = one<HTMLElement>("#pca-assignment-list");
  if (assignmentList) assignmentList.innerHTML = assignments.length
...
async function start(): Promise<void> {
  ...
  if (!accounts.some((account) => account.id === selectedAccountId)) selectedAccountId = accounts[0]?.id || "";
  renderAccountList(); renderDetail();
  one<HTMLElement>("#pca-status")!.hidden = true;
  document.body.classList.remove("portal-loading");
}

one("#pca-search")?.addEventListener("input", renderAccountList);
one("#pca-account-list")?.addEventListener("click", (event) => { const button = (event.target as HTMLElement).closest<HTMLButtonElement>("[data-account-id]"); if (!button) return; selectedAccountId = button.dataset.accountId || ""; renderDetail(); });
one("#pca-assignment-list")?.addEventListener("click", (event) => { const button = (event.target as HTMLElement).closest<HTMLButtonElement>("[data-activate-organization]"); if (!button) return; void (async () => { button.disabled = true; button.textContent = "Assigning…"; try { await invoke("activate-project-cards-for-account", { userId: selectedAccountId, organizationId: button.dataset.activateOrganization }); await loadData(); renderDetail(); } catch (error) { button.disabled = false; button.textContent = error instanceof Error ? error.message : "Assignment failed"; } })(); });
