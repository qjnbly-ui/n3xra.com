...
interface ResendRecord extends JsonObject {
  record?: string;
  name?: string;
  type?: string;
  ttl?: string;
...
interface ResendDomain extends JsonObject {
  id: string;
  name: string;
  status: string;
  region?: string;
...
const {
  ...
  sendJson,
  supabaseJson,
} = require("./_communications") as {
  clean: (value: unknown, limit?: number) => string;
  firstRow: (value: unknown) => JsonObject | null;
  requirePlatformAdmin: (req: AdminRequest) => Promise<PlatformAdminContext>;
  sendJson: (res: AdminResponse, status: number, body: unknown) => unknown;
...
function providerError(message: string, status = 502): ProviderError {
...
function requiredUuid(value: unknown, label: string): string {
  ...
  if (!UUID_PATTERN.test(normalized)) throw providerError(`${label} is invalid.`, 400);
  return normalized;
}

function requiredDomain(value: unknown): string {
  const normalized = clean(value, 253).toLowerCase().replace(/^https?:\/\//, "").replace(/\/$/, "");
  if (!DOMAIN_PATTERN.test(normalized)) throw providerError("Enter a valid sending domain without https:// or a path.", 400);
  return normalized;
}
...
function requiredText(value: unknown, label: string, minimum: number, maximum: number): string {
  ...
  if (normalized.length < minimum) throw providerError(`${label} is required.`, 400);
  return normalized;
}
...
function configuredApiKey(): string {
  ...
  if (!apiKey) throw providerError("Resend is not configured on this environment.", 503);
  return apiKey;
}

function providerCapabilities(): { providerAvailable: boolean; webhookAvailable: boolean } {
  return {
...
async function parseProviderResponse(response: Response): Promise<JsonObject> {
...
async function resendRequest(path: string, options: RequestInit = {}): Promise<JsonObject> {
...
function providerStatus(value: unknown): string {
  ...
  if (!supported.has(normalized)) throw providerError("Resend returned an unsupported domain status.");
  return normalized;
}

async function workspaceRow(workspaceId: string): Promise<JsonObject> {
  const workspace = firstRow(await supabaseJson(
    `communications_workspaces?select=id,organization_id,slug,program_name,sender_name,support_email,status&id=eq.${encodeURIComponent(workspaceId)}&limit=1`,
  ));
  if (!workspace) throw providerError("Communications workspace not found.", 404);
  return workspace;
}

async function domainRow(workspaceId: string): Promise<JsonObject | null> {
  return firstRow(await supabaseJson(
    `communications_sending_domains?select=id,workspace_id,domain,provider,provider_domain_id,status,created_at,updated_at&workspace_id=eq.${encodeURIComponent(workspaceId)}&provider=eq.resend&order=created_at.asc&limit=1`,
  ));
}

function publicDomain(domain: ResendDomain, stored: JsonObject | null): JsonObject {
  return {
    configured: true,
    domain: domain.name,
...
async function getProviderDomain(stored: JsonObject): Promise<ResendDomain> {
  const providerDomainId = requiredText(stored.provider_domain_id, "Resend domain identifier", 3, 200);
  const result = await resendRequest(`/${encodeURIComponent(providerDomainId)}`) as ResendDomain;
...
async function recordDomain(actorUserId: string, idempotencyKey: string, workspaceId: string, domain: ResendDomain): Promise<JsonObject> {
...
async function readState(workspaceId: string): Promise<JsonObject> {
...
async function createDomain(actorUserId: string, body: JsonObject): Promise<JsonObject> {
  ...
  await workspaceRow(workspaceId);
  if (await domainRow(workspaceId)) throw providerError("This workspace already has a Resend sending domain.", 409);
  const domain = requiredDomain(body.domain);
  const region = clean(body.region, 30) || "us-east-1";
  if (!REGIONS.has(region)) throw providerError("Choose a supported sending region.", 400);
...
async function refreshDomain(actorUserId: string, body: JsonObject, triggerVerification: boolean): Promise<JsonObject> {
  const workspaceId = requiredUuid(body.workspaceId, "Workspace");
  const idempotencyKey = requiredUuid(body.idempotencyKey, "Idempotency key");
  await workspaceRow(workspaceId);
  const stored = await domainRow(workspaceId);
  if (!stored) throw providerError("Add a Resend sending domain first.", 400);
  const providerDomainId = requiredText(stored.provider_domain_id, "Resend domain identifier", 3, 200);
...
async function activateDomain(actorUserId: string, body: JsonObject): Promise<JsonObject> {
  const workspaceId = requiredUuid(body.workspaceId, "Workspace");
  const idempotencyKey = requiredUuid(body.idempotencyKey, "Idempotency key");
  await workspaceRow(workspaceId);
  const stored = await domainRow(workspaceId);
  if (!stored) throw providerError("Add a Resend sending domain first.", 400);
  const domain = await getProviderDomain(stored);
  if (domain.status !== "verified") throw providerError("Resend has not verified every required DNS record yet.", 409);
  if (!providerCapabilities().webhookAvailable) throw providerError("Configure the signed Resend webhook before activating email.", 409);
...
async function sendTestEmail(body: JsonObject): Promise<JsonObject> {
  const workspaceId = requiredUuid(body.workspaceId, "Workspace");
  const subscriberId = requiredUuid(body.subscriberId, "Subscriber");
  const idempotencyKey = requiredUuid(body.idempotencyKey, "Idempotency key");
  const workspace = await workspaceRow(workspaceId);
  const stored = await domainRow(workspaceId);
  if (!stored || stored.status !== "verified") throw providerError("Activate a verified sending domain before sending a test.", 409);
  const localPart = clean(body.fromLocalPart, 64).toLowerCase();
  if (!LOCAL_PART_PATTERN.test(localPart)) throw providerError("From address name is invalid.", 400);
  const subject = requiredText(body.subject, "Subject", 1, 300);
  const message = requiredText(body.message, "Message", 1, 10000);
  const subscriber = firstRow(await supabaseJson(
    `communications_subscribers?select=id,email,email_status&workspace_id=eq.${encodeURIComponent(workspaceId)}&id=eq.${encodeURIComponent(subscriberId)}&limit=1`,
  ));
...
async function parseBody(req: AdminRequest): Promise<JsonObject> {
...
module.exports = async function handler(req: AdminRequest, res: AdminResponse): Promise<unknown> {
  ...
  try {
    ...
    if (operation === "create_domain") result = await createDomain(user.id, body);
    else if (operation === "refresh_domain") result = await refreshDomain(user.id, body, false);
    else if (operation === "verify_domain") result = await refreshDomain(user.id, body, true);
    else if (operation === "activate_email") result = await activateDomain(user.id, body);
    else if (operation === "send_test_email") result = await sendTestEmail(body);
    else throw providerError("Unknown Communications email operation.", 400);
    return sendJson(res, 200, result);
  } catch (caught) {
    const error = caught as ProviderError;
    const status = Number(error.status ?? 500);
    if (status >= 500) console.error("Communications email administration failed:", error);
