...
function claimsCompletedAction(value: string): boolean {
...
export class PhoneBuildConversation {
  ...
  private async useTool(name: string, args: Json, turn: number): Promise<{ data: Json; done?: boolean }> {
    const fields: Record<string, string[]> = { inspect_page: ["path"], get_status: [], propose_action: ["action", "instruction"], confirm_action: ["confirmation_id"], dismiss_action: [], cancel_request: [] };
    if (!fields[name] || Object.keys(args).some(key => !fields[name]!.includes(key))) throw new Error("Unknown tool or arguments.");
    if (name === "dismiss_action") { this.pending = undefined; return { data: { dismissed: true } }; }
    if (name === "get_status") {
    ...
    if (!instruction || args.confirmation_id !== instruction.id || instruction.turn >= turn || instruction.expires <= this.now()) return { data: { error: "No valid prior proposal to confirm. Discuss and propose the action first." } };
    ...
    try {
      if (instruction.action === "open") {
        ...
        const current = await this.rpc(`/v1/sessions/${this.sessionId}/phone-status`); this.state = current.session;
        if (this.state.state !== "ready") return { data: { error: "The workspace is not ready; the proposal was cleared. Check status." } };
  ...
  async poll(force = false) {
    ...
    try {
      ...
      if (this.pending || this.thinking || this.busy && !force) return;
