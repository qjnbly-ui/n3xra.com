...
export type PhoneIntent = "save" | "main" | "branch" | "close" | "cancel" | "status" | "help" | "clarify" | "edit";
export function phoneIntent(text: string): PhoneIntent {
  const raw = text.toLowerCase().replace(/[’]/g, "'").replace(/[.,!?]/g, " ").replace(/\s+/g, " ").trim();
  if (/\b(don't|do not|not yet|never mind)\b/.test(raw)) return "clarify";
  ...
  if (/^(?:save|push|publish|deploy)\b/.test(value)) {
    if (/\b(?:and|then|but|or)\b/.test(value)) return "clarify";
    if (/\bmain\b/.test(value) || /\b(?:live|production)\b/.test(value)) return "main";
    if (/\bbranch\b/.test(value)) return "branch";
    if (/^(?:save|push)(?: (?:it|this|that|everything|my work|the work|these changes|the changes|to github))?$/.test(value)) return "save";
    return "clarify";
  }
  if (/^(?:main|to main|the main branch|main branch)$/.test(value)) return "main";
  if (/^(?:branch|the branch|working branch|the working branch|to the branch)$/.test(value)) return "branch";
  if (/^(?:close|close (?:it|the project|the workspace|project|workspace))$/.test(value)) return "close";
  if (/^(?:stop|cancel)(?: (?:the |my )?(?:request|edit|change|current request))?$/.test(value)) return "cancel";
  if (/^(?:status|progress|what.*(?:doing|happening)|are you done|is it done|how is it going|what is the status)$/.test(value)) return "status";
  if (/^(?:help|thanks|thank you|what can you do|how.*save|what.*(?:branch|main)|what do you mean)/.test(value)) return "help";
...
export class PhoneBuildConversation {
  private sessionId = "";
  private pending: { action: "open" | "edit" | "save" | "publish" | "close"; text?: string } | undefined;
  private choosingSave = false;
  private lastState = "";
  ...
  private speak(text: string) { if (!this.disposed) { this.lastSpokenAt = this.now(); this.say(text); } }
  private valid() {
  ...
  async handle(text: string) {
    ...
    if (/\b(call me back|callback)\b/i.test(clean)) { this.choosingSave = false; this.pending = undefined; this.speak("Automatic callbacks are not enabled in this first test. You can call back, enter your PIN, and reconnect to your workspace."); return; }
    if (intent === "cancel") {
    ...
    if (NO.test(clean)) { this.choosingSave = false; this.pending = undefined; this.speak("Okay. I will not carry out that instruction."); return; }
    if (intent === "status") {
      this.choosingSave = false;
      this.pending = undefined;
    ...
    if (YES.test(clean)) {
      if (this.choosingSave) { this.speak("Which destination: the working branch, or main for the live website?"); return; }
  ...
  async poll(force = false) {
    ...
    try {
      ...
      if (this.pending || this.choosingSave || this.busy && !force) return;
