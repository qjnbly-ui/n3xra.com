...
type Store = (path:string,options?:Json)=>Promise<any>;
...
export function isTextedPin(body:string) { return /^\s*\d{4}\s*$/.test(body) || /\b(?:my\s+)?(?:pin|verification code|passcode)\s*(?:is|:|=)?\s*\d{4}\b/i.test(body); }
export async function issueSmsLink(threadId:string,phone:string,store:Store=supabaseJson,now=Date.now()) {
 if (!uuid(threadId) || !/^\+[1-9]\d{7,14}$/.test(phone)) throw Error("Invalid text conversation.");
 const existing=(await store(`nex_sms_sessions?thread_id=eq.${threadId}&select=requested_at&limit=1`))?.[0];
 if (existing && now-new Date(existing.requested_at).getTime()<60_000) return "Please use the sign-in link I just sent, or wait a minute before requesting another.";
...
export async function verifySmsLink(token:string,userId:string,store:Store=supabaseJson,credentialLookup=getCredentialByUser,now=Date.now(),consume=true) {
 if (!/^[a-f0-9]{64}$/.test(token) || !uuid(userId)) throw Error("Invalid verification link.");
 ...
 if (!row || !credential?.updated_at || !credential.phone_e164 || credential.phone_e164!==row.phone_e164) throw Error("Account does not match this text conversation.");
 if (!consume) return {phoneEnding:row.phone_e164.slice(-4)};
 ...
 if (!changed?.length) throw Error("Link already used or expired.");
 return {verified:true,expiresAt:until,phoneEnding:row.phone_e164.slice(-4)};
}
export async function revokeSmsAccess(phone:string,store:Store=supabaseJson) {
 if (!/^\+[1-9]\d{7,14}$/.test(phone)) return;
 await store(`nex_sms_sessions?phone_e164=eq.${encodeURIComponent(phone)}`,{method:'DELETE'});
}
export async function verifiedSmsUser(threadId:string,phone:string,store:Store=supabaseJson,callerLookup=getCallerAccount,credentialLookup=getCredentialByUser,now=Date.now()) {
 ...
 if (!row || row.phone_e164!==phone || !row.verified_user_id) return null;
 const caller=await callerLookup(phone),credential=await credentialLookup(row.verified_user_id);
 if (caller?.user_id!==row.verified_user_id || credential?.phone_e164!==phone || credential.updated_at!==row.credential_version) return null;
 return {id:row.verified_user_id,expiresAt:row.verified_until};
}
export async function smsAccountStatus(threadId:string,phone:string) {
 const user=await verifiedSmsUser(threadId,phone);
 if (!user) return "Please text VERIFY and sign in before requesting your account status.";
 ...
 const label:Record<string,string>={active:"active",trialing:"on a trial"};
 if (!label[status]) return "Your account status is unavailable by text. Please check your N3XRA account online.";
