...
export const requestBuildAgent: BuildAgent = async (messages, context, signal) => {
  ...
  const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: process.env.GROQ_RECEPTIONIST_MODEL || process.env.GROQ_ASK_MODEL || "openai/gpt-oss-120b",
      temperature: 0.2, max_tokens: 700, parallel_tool_calls: false, tool_choice: "auto", tools: phoneBuildTools,
      messages: [{ role: "system", content: phoneBuildRules }, { role: "system", content: `Current server context: ${JSON.stringify(context)}` }, ...messages] }),
