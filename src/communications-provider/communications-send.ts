...
type ApiRequest = { method?: string; headers?: JsonObject; body?: unknown };
type ApiResponse = { setHeader(name: string, value: string): void; status(code: number): ApiResponse; json(body: unknown): unknown };
type SupabaseJson = (path: string, options?: RequestInit) => Promise<any>;

const twilio = require("twilio") as (accountSid: string, authToken: string) => {
  messages: { create(input: JsonObject): Promise<JsonObject> };
};
const { authenticatedUser, clean, firstRow, sendJson, supabaseJson } = require("./_communications") as {
  authenticatedUser(req: ApiRequest): Promise<JsonObject | null>;
  clean(value: unknown, limit?: number): string;
  firstRow(value: unknown): JsonObject | null;
  sendJson(res: ApiResponse, status: number, body: JsonObject): unknown;
...
function requiredUuid(value: unknown, label: string): string {
  ...
  if (!UUID_PATTERN.test(normalized)) throw httpError(400, `${label} is invalid.`);
  return normalized;
}
...
function normalizeChannels(value: unknown): Array<"sms" | "email"> {
  const channels = [...new Set((Array.isArray(value) ? value : []).map((item) => clean(item, 20).toLowerCase()))]
    .filter((item): item is "sms" | "email" => item === "sms" || item === "email");
  if (!channels.length) throw httpError(400, "Choose text messages, email, or both.");
  return channels;
}
...
async function requireSender(req: ApiRequest, workspaceId: string, database: SupabaseJson = supabaseJson): Promise<{ user: JsonObject; workspace: JsonObject }> {
  ...
  if (!entitlement?.portal_enabled || !ACTIVE_ENTITLEMENTS.has(String(entitlement.status))) throw httpError(403, "This Communications subscription is not active.");
  if (!platformAdmin && (!access || !SEND_ROLES.has(String(access.role)))) throw httpError(403, "Editor or account administrator access is required to send messages.");
  if (workspace.status !== "active") throw httpError(409, "Communications delivery is not active for this organization yet.");
  return { user, workspace };
}

async function audience(database: SupabaseJson, workspaceId: string, topicId: string): Promise<JsonObject[]> {
  ...
  if (topicId) {
    ...
    if (!topic) throw httpError(400, "Choose an active subscriber topic.");
    ...
    allowedIds = new Set((choices || []).map((row: JsonObject) => String(row.subscriber_id)));
  }
  ...
  if (selected.length > MAX_RECIPIENTS) throw httpError(413, `Send to at most ${MAX_RECIPIENTS} subscribers at a time.`);
  return selected;
}

async function createOrLoadBroadcast(database: SupabaseJson, input: JsonObject): Promise<JsonObject> {
  ...
  if (existing) {
    if (existing.payload_hash !== input.payload_hash) throw httpError(409, "This send request was already used for different content.");
    return existing;
  }
  try {
    ...
    if (raced) {
      if (raced.payload_hash !== input.payload_hash) throw httpError(409, "This send request was already used for different content.");
      return raced;
    }
    throw error;
  }
...
async function claimRecipient(database: SupabaseJson, id: string): Promise<boolean> {
...
async function deliverEmail(database: SupabaseJson, context: JsonObject, recipient: JsonObject): Promise<string> {
...
async function deliverSms(database: SupabaseJson, context: JsonObject, recipient: JsonObject): Promise<string> {
  ...
  if (!accountSid || !authToken) throw httpError(503, "Text delivery is not configured.");
...
async function updateRecipient(database: SupabaseJson, id: string, status: "sent" | "failed", providerMessageId: string | null, message: string | null): Promise<void> {
...
async function handler(req: ApiRequest, res: ApiResponse): Promise<unknown> {
  if (req.method !== "POST") {
  ...
  try {
    ...
    if (!deliveries.length) throw httpError(409, "No subscribers in this audience have active consent for the selected channels.");

    const payloadHash = createHash("sha256").update(JSON.stringify({ workspaceId, topicId, channels: [...channels].sort(), subject, message })).digest("hex");
    ...
    if (!broadcast?.id) throw new Error("Broadcast could not be prepared.");
    ...
    const pendingRecipients = recipientRows as JsonObject[];
    for (let offset = 0; offset < pendingRecipients.length; offset += 5) {
      await Promise.all(pendingRecipients.slice(offset, offset + 5).map(async (row) => {
        if (!await claimRecipient(supabaseJson, row.id)) return;
        ...
        try {
          ...
          failedCount += 1;
          await updateRecipient(supabaseJson, row.id, "failed", null, clean((caught as Error).message || "Delivery failed.", 1000));
        }
      }));
    }
    ...
    if (status >= 500) console.error("Communications client send failed:", error);
    return sendJson(res, status >= 400 && status < 600 ? status : 500, { error: status >= 500 ? "Messages could not be sent right now." : error.message });
  }
}

export = Object.assign(handler, { audience, normalizeChannels, requireSender, smsSegments });
