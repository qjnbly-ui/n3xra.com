...
Commands: `status`, `cancel`, `save`, `close`. Save asks whether to use the working branch or main, then requires confirmation. “Save to main” selects main and explains that it triggers live publishing before asking for confirmation. Close requires confirmation and the worker verifies that all work is saved on GitHub. Control requests and unclear conversation are never sent as edit prompts. Phone disconnect stops the phone progress timer; it does not cancel, repeat, publish, or delete the task. The worker's existing idle rules still apply.
...
- The phone token cannot access raw pushes, login, model management, previews, or arbitrary worker endpoints.
