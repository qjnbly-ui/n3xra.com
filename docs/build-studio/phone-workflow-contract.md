...
- Plain save uses the remembered destination, main by default. Explicit draft chooses the working branch. A future intention such as "eventually main" remembers a preference without publishing now.
- Save actually invokes the selected server operation. A save while editing queues once, runs after completion, and can be canceled. A lost mutation response must not be automatically replayed.
