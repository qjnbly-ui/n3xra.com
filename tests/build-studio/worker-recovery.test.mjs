...
test('worker restores sessions and Codex threads; edits, previews, checkpoints and pushes survive restart', { timeout: 40000 }, async () => {
  ...
  await executable('codex', `const fs=require('node:fs'),path=require('node:path'),{randomUUID}=require('node:crypto');const threads=new Map();let initialized=false;require('node:readline').createInterface({input:process.stdin}).on('line',line=>{const q=JSON.parse(line);if(!('id'in q))return;let result={};let error;const p=q.params;const send=x=>process.stdout.write(JSON.stringify(x)+'\\n');if(q.method==='initialize')initialized=true;else if(!initialized)error={message:'Not initialized'};else if(q.method==='account/read')result={account:{type:'chatgpt'}};else if(q.method==='thread/start'){const id=randomUUID();fs.mkdirSync(process.env.CODEX_HOME,{recursive:true});fs.writeFileSync(path.join(process.env.CODEX_HOME,id),p.cwd);threads.set(id,p.cwd);result={thread:{id}};}else if(q.method==='thread/resume'){try{threads.set(p.threadId,fs.readFileSync(path.join(process.env.CODEX_HOME,p.threadId),'utf8'));result={thread:{id:p.threadId}};}catch{error={message:'thread not found: '+p.threadId};}}else if(q.method==='turn/start'){if(!threads.has(p.threadId))error={message:'thread not found: '+p.threadId};else{const id=randomUUID();result={turn:{id}};setTimeout(()=>{const failed=p.input[0].text.includes('FAIL_TEST');if(!failed)fs.writeFileSync(path.join(p.cwd,process.env.N3XRA_TEST_ASTRO_MODULES?'src/pages/index.astro':'index.html'),'<h1>Ready for liftoff</h1><svg aria-label="Rocket"></svg>');send({method:'item/agentMessage/delta',params:{threadId:p.threadId,turnId:id,itemId:'progress',delta:'Inspecting the page.'}});send({method:'item/agentMessage/delta',params:{threadId:p.threadId,turnId:id,itemId:'answer',delta:'Added the rocket.'}});send({method:'turn/completed',params:{threadId:p.threadId,turn:{id,status:failed?'failed':'completed',...(failed?{error:{message:'Fixture model failure'}}:{})}}});},100);}}send({id:q.id,...(error?{error}:{result})});});`);
  ...
  const api = createServer(async (req, res) => {
    ...
    else if(url.pathname.endsWith('/website_build_events')){if(req.method==='POST'){const row={...body,id:events.length+1};events.push(row);result=[row];}else result=events.filter(matches);}
  ...
  try {
    ...
    assert.equal(events.find(e=>e.event_type==='agent_message').message,'Inspecting the page.\n\nAdded the rocket.');
    ...
    await waitFor(()=>Promise.resolve(events.find(e=>e.event_type==='error'&&e.message==='Fixture model failure')));
