...
test("the standard client website template is directly deployable", async () => {
  const [packageJson, config, page] = await Promise.all([
  ...
  assert.match(page, /Private website preview/);
});
