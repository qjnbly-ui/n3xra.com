...
test("editors can correct a speaker label across the transcript and saved document", async () => {
  ...
  assert.doesNotMatch(recordings, /recordingDetailModal\.addEventListener\("click"/);
  assert.doesNotMatch(allRecordings, /recordingDetailModal\.addEventListener\("click"/);
  assert.doesNotMatch(recordings, /event\.key === "Escape" && recordingDetailModal/);
  assert.match(correctionModal, /Choose a speaker/);
  assert.match(correctionModal, /Correct name/);
  assert.match(correctionModal, /Save speaker name/);
  ...
  assert.match(correctionModal, /class="settings-modal-close"/);
  assert.doesNotMatch(correctionModal, /event\.target === modal/);
  assert.doesNotMatch(correctionModal, /close\(true\)/);
  assert.match(correctionModal, /Speaker name saved/);
