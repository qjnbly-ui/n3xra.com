...
test('clear edit reaches builder; later save publishes exactly once without destination questions',async()=>{
 ...
 await f.handle('Okay save',action('save'));
...
test('later main sets preference without publishing; explicit draft is remembered for later saves',async()=>{
 ...
 await f.handle('Save a draft',action('draft'));assert.equal(f.calls.filter(c=>c.path.endsWith('/save')).length,1);
 await f.handle('Save',action('save'));assert.equal(f.calls.filter(c=>c.path.endsWith('/save')).length,2);assert.equal(f.calls.filter(c=>c.path.endsWith('/publish')).length,0);
 }finally{f.flow.dispose();}
});
test('save during an edit waits for completion and does not interrupt caller speech',async()=>{
 ...
 await f.handle('Save it',action('save'));assert.equal(f.calls.filter(c=>c.path.endsWith('/publish')).length,0);
...
test('cancelled queued save never publishes; a lost publish response is not retried by polling',async()=>{
 const f=fixture();try{await f.open();await f.handle('Edit',action('edit','Add a bear.'));await f.handle('Save',action('save'));
 ...
 f.fail('/publish');await f.handle('Save now',action('save'));f.fail('');f.tick();await f.flow.poll();await f.flow.poll();assert.equal(f.calls.filter(c=>c.path.endsWith('/publish')).length,1);assert.match(f.speech.join(' '),/could not confirm/);
 }finally{f.flow.dispose();}
});
...
test('caller speech during status lookup stops a stale save before it is sent',async()=>{
 const f=fixture();try{await f.open();const waiting=f.holdStatus();const saving=f.handle('Save',action('save'));const release=await waiting;
