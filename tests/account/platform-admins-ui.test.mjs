...
test("existing accounts receive administrator access immediately without an invitation", async () => {
  ...
  assert.doesNotMatch(html, /id="platform-dashboard-preview-dialog"/);
  assert.match(controller, /\/account\/\?admin_preview=\$\{encodeURIComponent\(String\(item\.user_id\)\)\}/);
  assert.match(platformAdminFunction, /action === "get-platform-admin-structure-preview"/);
  assert.match(platformAdminFunction, /if \(!isOwnerAdmin\(platformAdmin\)\)/);
  assert.match(platformAdminFunction, /\.eq\("account_user_id", accountUserId\)/);
  assert.match(platformAdminFunction, /organization_product_entitlements/);
  assert.match(platformAdminFunction, /records: product\("records", membershipOrganizationIds\)/);
  assert.match(platformAdminFunction, /files_assets: product\("files_assets"\)/);
