...
test("friendly card URLs and separate customer and admin entry points are connected", async () => {
  ...
  assert.doesNotMatch(admin, /id="contact-card-modal"/);
  assert.doesNotMatch(admin, /id="contact-card-modal-close"/);
  assert.doesNotMatch(admin, /id="contact-card-modal-backdrop"/);
  assert.match(admin, /\/card\/admin\.js\?v=10/);
  assert.match(admin, /\/card\/card\.css\?v=14/);
  assert.match(admin, /\/account\/admin\/product-shell\.js\?v=19/);
  assert.match(admin, /contact-cards-admin\.css\?v=6/);
  assert.match(admin, /id="admin-card-links"/);
  assert.match(admin, /data-admin-media-input="profile"/);
  assert.match(adminLogic, /async function selectCard/);
  assert.match(adminLogic, /function saveErrorMessage/);
  assert.match(adminLogic, /details\.code === "23505"/);
  assert.doesNotMatch(adminLogic, /function openModal/);
  assert.doesNotMatch(adminLogic, /event\.key === "Escape"/);
  assert.match(adminLogic, /async function saveBeforeSelectionChange/);
  ...
  assert.doesNotMatch(adminStyles, /\.contact-card-modal \{ position:fixed/);
  assert.match(adminStyles, /\.contact-card-admin-form \{ min-height:100%/);
