...
function agencyOptions(items, selected, placeholder) {
...
function editableAgencyRow(kind, name = "") {
...
function syncIncident() {
    ...
    editorState.fireAgency = document.querySelector("#fire-agency")?.value || "";
    editorState.protectionAgency = document.querySelector("#protection-agency")?.value || "";
...
function updateSummary() {
    ...
    document.querySelectorAll("[data-line]").forEach((row) => {
        ...
        if (hoursInput)
            hoursInput.value = hours ? hours.toFixed(2) : "";
        if (lineTotal && line)
    ...
    const elapsedInput = document.querySelector("#elapsed-hours");
    if (elapsedInput)
        elapsedInput.value = hours.toFixed(2);
...
function saveInvoice(requireComplete = false) {
    ...
    if (requireComplete && (!editorState.incidentNumber || !editorState.incidentName || !editorState.fireAgency || !editorState.protectionAgency)) {
        if (message)
            message.textContent = "Add the incident number, incident name, fire agency, and protection agency before printing.";
        return false;
    }
    ...
    if (message)
        message.textContent = requireComplete ? "Invoice saved. Choose Print or Save as PDF in the print dialog." : "Invoice saved.";
    return true;
}
function bindEditor() {
    ...
    document.querySelector(".invoice-content")?.addEventListener("click", (event) => {
        ...
        if (button.id === "save-invoice")
            saveInvoice(false);
        if (button.id === "print-invoice" && saveInvoice(true))
            window.print();
    });
}
function bindRates() {
    document.querySelector(".invoice-content")?.addEventListener("click", (event) => {
        ...
        const agency = button.dataset.addAgency;
        if (group)
            document.querySelector(`[data-rate-list="${group}"]`)?.insertAdjacentHTML("beforeend", editableRateRow(group));
        if (agency)
            document.querySelector(`[data-agency-list="${agency}"]`)?.insertAdjacentHTML("beforeend", editableAgencyRow(agency));
    ...
    document.querySelector("#save-rates")?.addEventListener("click", () => {
        const next = { apparatus: {}, personnel: {}, fireAgencies: [], protectionAgencies: [] };
        ...
        document.querySelectorAll("[data-agency-row]").forEach((row) => {
            const kind = row.dataset.agencyRow;
            const name = (row.querySelector("[data-agency-name]")?.value || "").trim();
        ...
        if (status)
            status.textContent = "Rate library saved.";
    });
