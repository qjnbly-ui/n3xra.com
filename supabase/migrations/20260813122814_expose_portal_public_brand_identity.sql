    ...
    jsonb_build_object(
      ...
      'logo_url', case
        when logo_version.status = 'published'
          and logo_version.storage_bucket = 'website-assets-public'
          and logo_version.public_url ~ '^https://'
        then logo_version.public_url
        else null
      end,
      'favicon_url', case
        when favicon_version.status = 'published'
          and favicon_version.storage_bucket = 'website-assets-public'
          and favicon_version.public_url ~ '^https://'
        then favicon_version.public_url
        else null
      end,
      'website_url', cw.live_url,
