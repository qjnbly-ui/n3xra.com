drop function if exists public.platform_connect_website_client_organization(uuid, uuid);
...
begin
  ...
  if input_owner_user_id is not null then
    if not exists (
      select 1
      from auth.users user_record
      where user_record.id = input_owner_user_id
        and user_record.email_confirmed_at is not null
        and (user_record.banned_until is null or user_record.banned_until <= now())
    ) then
      raise exception 'Choose an active, confirmed N3XRA account as the organization owner.';
    end if;

    if target_owner_user_id is not null and target_owner_user_id <> input_owner_user_id then
      raise exception 'The selected owner does not match the website owner already assigned.';
    end if;

    target_owner_user_id := input_owner_user_id;
    owner_count := 1;
  ...
  set client_user_id = target_owner_user_id,
      updated_at = now()
  where managed_website_id = website_record.id
...
revoke all on function public.platform_connect_website_client_organization(uuid, uuid, uuid)
from public, anon;
grant execute on function public.platform_connect_website_client_organization(uuid, uuid, uuid)
to authenticated;
;
