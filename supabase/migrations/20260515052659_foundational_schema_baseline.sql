-- Foundational baseline reconstructed from the last aggregate schema snapshot
-- committed before the production migration ledger began.
-- Source commit: 42908bd9f38c35204c3045b4ac6e5912d33641d3
-- Source commit time: 2026-05-14T22:26:59-07:00 (2026-05-15T05:26:59Z)
--
-- Fresh databases apply this migration before the exact production-ledger
-- sequence. Existing production must mark this baseline version as applied
-- during an explicitly approved deployment; it must not be replayed over
-- the live schema.
...
create table if not exists public.organizations (
  ...
  public_embed_token text unique default encode(extensions.gen_random_bytes(12), 'hex'),
...
-- These legacy per-user document policies exist in production outside the
-- hosted migration ledger. Recreate them in the fresh-database baseline so
-- the later production migration that scopes them to authenticated can replay.
