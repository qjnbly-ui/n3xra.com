...
create unique index if not exists loan_accounts_one_active_per_user on public.loan_accounts(user_id) where status = 'active';
create index if not exists loan_accounts_user_id_idx on public.loan_accounts(user_id);
create index if not exists loan_payments_account_date_idx on public.loan_payments(loan_account_id, payment_date, created_at);

drop trigger if exists loan_accounts_set_updated_at on public.loan_accounts;
create trigger loan_accounts_set_updated_at before update on public.loan_accounts for each row execute function public.set_updated_at();
...
create policy "loan_accounts_owner_or_admin_select" on public.loan_accounts for select to authenticated using ((select auth.uid()) = user_id or (select public.is_platform_admin()));
drop policy if exists "loan_accounts_admin_insert" on public.loan_accounts;
create policy "loan_accounts_admin_insert" on public.loan_accounts for insert to authenticated with check ((select public.is_platform_admin()));
drop policy if exists "loan_accounts_admin_update" on public.loan_accounts;
create policy "loan_accounts_admin_update" on public.loan_accounts for update to authenticated using ((select public.is_platform_admin())) with check ((select public.is_platform_admin()));

drop policy if exists "loan_payments_owner_or_admin_select" on public.loan_payments;
create policy "loan_payments_owner_or_admin_select" on public.loan_payments for select to authenticated using ((select public.is_platform_admin()) or exists (select 1 from public.loan_accounts where loan_accounts.id = loan_payments.loan_account_id and loan_accounts.user_id = (select auth.uid())));
drop policy if exists "loan_payments_owner_or_admin_insert" on public.loan_payments;
create policy "loan_payments_owner_or_admin_insert" on public.loan_payments for insert to authenticated with check ((select public.is_platform_admin()) or exists (select 1 from public.loan_accounts where loan_accounts.id = loan_payments.loan_account_id and loan_accounts.user_id = (select auth.uid())));
drop policy if exists "loan_payments_owner_or_admin_update" on public.loan_payments;
create policy "loan_payments_owner_or_admin_update" on public.loan_payments for update to authenticated using ((select public.is_platform_admin()) or exists (select 1 from public.loan_accounts where loan_accounts.id = loan_payments.loan_account_id and loan_accounts.user_id = (select auth.uid()))) with check ((select public.is_platform_admin()) or exists (select 1 from public.loan_accounts where loan_accounts.id = loan_payments.loan_account_id and loan_accounts.user_id = (select auth.uid())));;
