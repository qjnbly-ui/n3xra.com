create table public.website_projects (
  ...
  constraint website_projects_name_check check (char_length(btrim(name)) between 1 and 180),
  constraint website_projects_status_check check (status in ('pending', 'active', 'waiting_on_client', 'on_hold', 'ready_to_launch', 'launched', 'cancelled', 'archived')),
  constraint website_projects_stage_check check (current_stage in ('agreement', 'billing', 'onboarding', 'production', 'client_review', 'launch', 'ongoing')),
  constraint website_projects_progress_check check (progress_percent between 0 and 100),
  constraint website_projects_dates_check check (target_start_date is null or target_launch_date is null or target_launch_date >= target_start_date)
);
...
create table public.website_project_milestones (
  ...
  constraint website_project_milestones_stage_check check (stage in ('agreement', 'billing', 'onboarding', 'production', 'client_review', 'launch', 'ongoing')),
  constraint website_project_milestones_sequence_check check (sequence_number between 1 and 7),
  constraint website_project_milestones_label_check check (char_length(btrim(label)) between 1 and 100),
  constraint website_project_milestones_status_check check (status in ('not_started', 'available', 'in_progress', 'blocked', 'complete', 'not_applicable')),
  constraint website_project_milestones_completion_check check ((status = 'complete' and completed_at is not null) or (status <> 'complete'))
);

create index website_projects_client_created_idx on public.website_projects (client_user_id, created_at desc);
create index website_projects_status_updated_idx on public.website_projects (status, updated_at desc);
create index website_projects_owner_admin_idx on public.website_projects (owner_admin_user_id) where owner_admin_user_id is not null;
create index website_projects_created_by_idx on public.website_projects (created_by_user_id) where created_by_user_id is not null;
create index website_project_milestones_status_idx on public.website_project_milestones (status, updated_at desc);
...
create trigger website_project_milestones_set_updated_at before update on public.website_project_milestones for each row execute function public.set_updated_at();
...
as $$
begin
  insert into public.website_project_milestones (project_id, stage, sequence_number, label, status, client_description)
  values
...
create trigger website_projects_initialize_milestones after insert on public.website_projects for each row execute function private.initialize_website_project_milestones();
...
begin
  ...
  set progress_percent = round((completed_stage_count::numeric / 6) * 100)::integer,
...
create trigger website_project_milestones_recalculate_project after update of status on public.website_project_milestones for each row execute function private.recalculate_website_project_progress();
...
begin
  ...
  select * into request_row from public.website_service_requests where id = new.request_id;

  insert into public.website_projects (
    request_id, proposal_id, client_user_id, name, status, current_stage, progress_percent,
    target_launch_date, client_summary, created_by_user_id
  ) values (
    new.request_id, new.id, new.client_user_id, request_row.business_name, 'pending', 'agreement', 0,
    request_row.target_launch_date, 'Your proposal is approved. Project setup is ready to begin.', (select auth.uid())
...
create trigger website_proposals_ensure_project after insert or update of status on public.website_proposals
for each row when (new.status = 'approved') execute function private.ensure_website_project_for_approved_proposal();
...
begin
  select id into project_row_id from public.website_projects where proposal_id = new.proposal_id;
  if project_row_id is null then return new; end if;
  ...
  where project_id = project_row_id and stage = 'onboarding';
  return new;
end;
$$;
...
create trigger website_onboardings_sync_project_milestone after insert or update of status on public.website_onboardings
...
begin
  ...
  then raise exception 'Website project ownership is read-only after creation.';
  end if;
  if new.status = 'launched' and old.status <> 'launched' and new.launched_at is null then new.launched_at := now(); end if;
  return new;
end;
$$;
...
create trigger website_projects_protect_identity before update on public.website_projects for each row execute function private.protect_website_project_identity();
...
begin
  ...
  then raise exception 'Website project milestone identity is read-only.';
  end if;
...
create trigger website_project_milestones_protect_identity before update on public.website_project_milestones for each row execute function private.protect_website_project_milestone_identity();
...
create policy "website_projects_admin_update" on public.website_projects for update to authenticated
using ((select public.is_platform_admin())) with check ((select public.is_platform_admin()));
create policy "website_project_milestones_select" on public.website_project_milestones for select to authenticated
using (exists (
  select 1 from public.website_projects project
  where project.id = project_id
    and (project.client_user_id = (select auth.uid()) or (select public.is_platform_admin()))
));
create policy "website_project_milestones_admin_update" on public.website_project_milestones for update to authenticated
using ((select public.is_platform_admin())) with check ((select public.is_platform_admin()));
...
select proposal.request_id, proposal.id, proposal.client_user_id, request.business_name, 'pending', 'agreement', 0,
  request.target_launch_date, 'Your proposal is approved. Project setup is ready to begin.', proposal.created_by_user_id
from public.website_proposals proposal
join public.website_service_requests request on request.id = proposal.request_id
where proposal.status = 'approved'
on conflict (proposal_id) do nothing;

update public.website_project_milestones milestone
set status = case onboarding.status
      when 'not_started' then 'available'
      when 'in_progress' then 'in_progress'
      when 'submitted' then 'in_progress'
      when 'needs_changes' then 'in_progress'
...
join public.website_onboardings onboarding on onboarding.proposal_id = project.proposal_id
where milestone.project_id = project.id and milestone.stage = 'onboarding';;
