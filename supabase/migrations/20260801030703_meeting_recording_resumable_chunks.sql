...
alter table public.meeting_recordings add constraint meeting_recordings_status_check check (status in ('created','recording','interrupted','recorded','uploading','finalizing','uploaded','transcribing','ready','failed'));
...
create trigger meeting_recording_chunks_set_updated_at before update on public.meeting_recording_chunks for each row execute function public.set_updated_at();
drop trigger if exists meeting_recording_interruptions_set_updated_at on public.meeting_recording_interruptions;
create trigger meeting_recording_interruptions_set_updated_at before update on public.meeting_recording_interruptions for each row execute function public.set_updated_at();
...
create policy "meeting_recording_chunks_select_policy" on public.meeting_recording_chunks for select to authenticated using (public.can_view_records_recordings(organization_id));
create policy "meeting_recording_chunks_insert_policy" on public.meeting_recording_chunks for insert to authenticated with check (
  created_by_user_id = (select auth.uid()) and public.can_change_records_recordings(organization_id)
...
create policy "meeting_recording_chunks_update_policy" on public.meeting_recording_chunks for update to authenticated
using (public.can_change_records_recordings(organization_id)) with check (public.can_change_records_recordings(organization_id));
create policy "meeting_recording_chunks_delete_policy" on public.meeting_recording_chunks for delete to authenticated using (public.can_change_records_recordings(organization_id));

create policy "meeting_recording_interruptions_select_policy" on public.meeting_recording_interruptions for select to authenticated using (public.can_view_records_recordings(organization_id));
create policy "meeting_recording_interruptions_insert_policy" on public.meeting_recording_interruptions for insert to authenticated with check (
  created_by_user_id = (select auth.uid()) and public.can_change_records_recordings(organization_id)
  and exists (select 1 from public.meeting_recordings mr where mr.id = meeting_recording_id and mr.organization_id = organization_id)
);
create policy "meeting_recording_interruptions_update_policy" on public.meeting_recording_interruptions for update to authenticated
using (public.can_change_records_recordings(organization_id)) with check (public.can_change_records_recordings(organization_id));
create policy "meeting_recording_interruptions_delete_policy" on public.meeting_recording_interruptions for delete to authenticated using (public.can_change_records_recordings(organization_id));;
