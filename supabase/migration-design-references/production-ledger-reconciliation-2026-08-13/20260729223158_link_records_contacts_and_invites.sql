...
alter table public.organization_invites
  drop constraint if exists organization_invites_recipient_email_check;

alter table public.organization_invites
  add constraint organization_invites_recipient_email_check
  check (
...
set linked_user_id = membership.user_id
from public.organization_memberships as membership
join public.profiles as profile
  on profile.id = membership.user_id
where membership.organization_id = contact.organization_id
  and lower(trim(profile.email)) = lower(trim(contact.email))
  and contact.linked_user_id is distinct from membership.user_id;
