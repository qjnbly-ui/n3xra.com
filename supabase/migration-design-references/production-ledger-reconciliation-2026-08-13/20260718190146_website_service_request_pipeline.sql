create table if not exists public.website_service_requests (
  ...
  constraint website_service_requests_project_type_check
    check (project_type in ('new_website', 'redesign', 'landing_page', 'ecommerce', 'maintenance', 'other')),
  constraint website_service_requests_status_check
    check (status in ('draft', 'submitted', 'reviewing', 'needs_info', 'qualified', 'declined', 'converted', 'archived')),
  constraint website_service_requests_email_check
    check (contact_email ~* '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$')
);

create index if not exists website_service_requests_user_created_idx
on public.website_service_requests (user_id, created_at desc);

create index if not exists website_service_requests_status_created_idx
on public.website_service_requests (status, created_at desc);

create index if not exists website_service_requests_reviewed_by_idx
on public.website_service_requests (reviewed_by_user_id);

drop trigger if exists website_service_requests_set_updated_at on public.website_service_requests;
create trigger website_service_requests_set_updated_at
before update on public.website_service_requests
...
create policy "website_service_requests_select"
...
create policy "website_service_requests_insert"
on public.website_service_requests
for insert
to authenticated
with check (
...
using (
  ...
  or (user_id = auth.uid() and status in ('draft', 'submitted', 'needs_info'))
)
with check (
  public.is_platform_admin()
  or (
