...
Checkpoint identity requires `N3XRA_BUILD_GIT_AUTHOR_NAME` and `N3XRA_BUILD_GIT_AUTHOR_EMAIL` in the coordinator's settings. Use a platform-authorized GitHub account linked to the Vercel team; its GitHub-provided noreply address avoids publishing a private email. The coordinator explicitly passes both author and committer identity to each checkpoint, including previously opened workspaces. Missing configuration stops checkpoint creation before staging. Do not use a generic unlinked address: GitHub accepts those commits but Vercel can block their deployments.
