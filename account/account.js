...
async function loadNotificationRecipients() {
  if (!supabase || !isPlatformAdminEmail(currentSession?.user?.email)) return;
  const product = notificationProductInput.value;
  setInlineStatus(notificationStatus, "Loading accounts...");
  ...
  if (error || data?.error) {
    setInlineStatus(notificationStatus, error?.message || data?.error || "Unable to load accounts.", "error");
    return;
  }
  ...
  selectedNotificationEmails = new Set();
  renderNotificationRecipients();
  setInlineStatus(notificationStatus, `${notificationRecipients.length} account${notificationRecipients.length === 1 ? "" : "s"} loaded. Select recipients before reviewing.`, "success");
}
...
function setNotificationReviewOpen(isOpen) {
  ...
  if (!isOpen) {
    ...
    setInlineStatus(notificationReviewStatus, "");
  }
}
...
function handleNotificationReview() {
  try {
    ...
    setInlineStatus(notificationStatus, error.message || "Review failed.", "error");
  }
}

async function sendNotificationEmail() {
  if (!pendingNotificationPayload || !supabase || !isPlatformAdminEmail(currentSession?.user?.email)) return;
  notificationSendButton.disabled = true;
  setInlineStatus(notificationReviewStatus, "Sending update email...");
  ...
  if (error || data?.error) {
    setInlineStatus(notificationReviewStatus, error?.message || data?.error || "Unable to send update email.", "error");
    return;
  }

  setInlineStatus(notificationReviewStatus, `${data.sentCount || 0} sent${data.failedCount ? `, ${data.failedCount} failed` : ""}.`, data.failedCount ? "error" : "success");
  setInlineStatus(notificationStatus, `${data.sentCount || 0} update email${Number(data.sentCount || 0) === 1 ? "" : "s"} sent.`, data.failedCount ? "error" : "success");
}
