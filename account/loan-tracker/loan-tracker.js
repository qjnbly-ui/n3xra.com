...
function createPrintReportWindow() {
  ...
  if (useMobileReportWindow) {
    ...
    reportWindow = reportFrame.contentWindow;
  }
  if (!reportWindow) {
    reportFrame?.remove();
...
function loanPrintDocument({ title, summaryCards, sectionTitle, note, columns, headers, rows, paymentHistory = false }) {
  const summary = summaryCards.map(({ label, value }) => `<div class="card"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("");
  const colgroup = columns.map((width) => `<col style="width:${width}">`).join("");
  const heading = headers.map((label) => `<th>${escapeHtml(label)}</th>`).join("");
  return `<!doctype html><html><head><title>N3XRA Loan Tracker - ${escapeHtml(title)}</title><style>@page{size:letter portrait;margin:.35in}:root{color-scheme:light;font-family:Arial,Helvetica,sans-serif}body{color:#17211b;margin:0;font-size:8pt}.brand{display:flex;align-items:center;gap:8px;border-bottom:2px solid #1e5c43;padding-bottom:7px;margin-bottom:8px}.brand img{width:22px;height:22px;object-fit:contain}.brand strong{color:#1e5c43;font-size:11pt;letter-spacing:.08em}h1{margin:0;font-size:17pt;letter-spacing:-.03em}h2{margin:10px 0 4px;font-size:10pt;color:#1e5c43}.meta{display:flex;justify-content:space-between;color:#68736c;font-size:7pt;margin-top:2px}.summary{display:grid;grid-template-columns:repeat(${summaryCards.length},1fr);gap:5px;margin:9px 0}.card{border:1px solid #dddcd2;border-radius:4px;padding:5px}.card span{display:block;color:#68736c;font-size:5.8pt;text-transform:uppercase;letter-spacing:.04em}.card strong{display:block;margin-top:2px;font-size:8.5pt}table{width:100%;border-collapse:collapse;table-layout:fixed}thead{display:table-header-group}tr{break-inside:avoid}th{background:#1e5c43;color:white;text-align:left;font-size:5.8pt;text-transform:uppercase;letter-spacing:.015em}th,td{padding:3.5px 2px;border-bottom:1px solid #dddcd2;white-space:nowrap;overflow:hidden;text-overflow:clip}td:nth-child(n+3),th:nth-child(n+3){text-align:right;font-variant-numeric:tabular-nums}.payment-history td:nth-child(2),.payment-history th:nth-child(2),.payment-history td:nth-child(3),.payment-history th:nth-child(3),.payment-history td:last-child,.payment-history th:last-child{text-align:left}.payment-history td:last-child{white-space:normal;overflow-wrap:anywhere}.voided td{text-decoration:line-through;color:#8f332f;background:#faf0ef}.note{color:#68736c;font-size:6.5pt;margin:0 0 5px}</style></head><body><div class="brand"><img src="${window.location.origin}/assets/n3xra_logo_transparent_small.png" alt="N3XRA"><strong>N3XRA</strong></div><h1>Loan Tracker</h1><div class="meta"><span>${escapeHtml(account.borrower_name || "Dave Wilson")} · ${escapeHtml(account.lender_name || "Vibrant Credit Union")}</span><span>Generated ${escapeHtml(dateLabel(new Date().toISOString()))}</span></div><div class="summary">${summary}</div><h2>${escapeHtml(sectionTitle)}</h2><p class="note">${escapeHtml(note)}</p><table${paymentHistory ? ' class="payment-history"' : ""}><colgroup>${colgroup}</colgroup><thead><tr>${heading}</tr></thead><tbody>${rows}</tbody></table></body></html>`;
}
...
function exportPdf() {
  ...
  reportWindow.document.write(loanPrintDocument({
    title: "Amortization Schedule",
    summaryCards: [
      { label: "Current balance", value: moneyCents(anchor.balanceCents) },
      { label: "Monthly payment", value: money(account.planned_monthly_payment) },
      { label: "Estimated payoff", value: dateLabel(summary.payoffDate, true) },
      { label: "Payments remaining", value: summary.payments },
      { label: "Estimated interest", value: moneyCents(summary.totalInterestCents) },
    ],
    sectionTitle: "Amortization Schedule",
    note: "Estimated schedule based on the current balance and planned payment. Official lender records take priority.",
    columns: ["5%", "15%", "17%", "15%", "15%", "15%", "18%"],
    headers: ["#", "Payment date", "Beginning balance", "Payment", "Interest", "Principal", "Ending balance"],
    rows,
  }));
...
function printPaymentHistory() {
  ...
  reportWindow.document.write(loanPrintDocument({
    ...
    summaryCards: [
      { label: "Period", value: year || "All years" },
      { label: "Recorded payments", value: activePayments.length },
      { label: "Total paid", value: moneyCents(totalPaid) },
      { label: "Principal", value: moneyCents(totalPrincipal) },
      { label: "Interest", value: moneyCents(totalInterest) },
    ],
    sectionTitle: "Payment History",
    note: "Voided records remain visible for audit history. An asterisk marks an official balance supplied with a payment.",
    columns: ["5%", "11%", "11%", "11%", "10%", "10%", "14%", "28%"],
    headers: ["#", "Due date", "Paid date", "Amount", "Interest", "Principal", "Balance", "Notes"],
    rows,
    paymentHistory: true,
  }));
  reportWindow.document.close();
