 ...
 */
export function renderNotificationMessageHtml(input) {
...
export function notificationMessageToPlainText(input) {
    return String(input || "")
        .replace(/\r\n?/g, "\n")
        .replace(HTTP_LINK_PATTERN, (_match, label, url) => `${label} (${url})`)
        .replace(/^#{1,3}\s+/gm, "")
        .replace(/^[-*]\s+/gm, "• ")
        .replace(/^>\s?/gm, "")
        .replace(/\*\*([^*\n]+)\*\*/g, "$1")
