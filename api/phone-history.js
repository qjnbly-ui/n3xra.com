...
async function handler(req, res) {
    ...
    try {
        ...
        for (const request of requests || []) {
            const following = await (0, _phone_records_1.phoneRecordStore)(`website_build_events?session_id=eq.${request.session_id}&id=gt.${request.id}&event_type=in.(user_message,agent_message,error,push,status)&select=id,event_type,message,technical_notes,created_at&order=id.asc&limit=100`);
            const boundary = (following || []).findIndex((event) => event.event_type === "user_message");
            const work = boundary < 0 ? following || [] : following.slice(0, boundary);
            const next = work.find((event) => ["agent_message", "error"].includes(event.event_type));
            builds.push({ id: request.id, created_at: request.created_at, instruction: (0, _phone_records_1.redactPhoneText)(request.message),
                ...
                work: work.map((event) => ({ kind: event.event_type, message: (0, _phone_records_1.redactPhoneText)(event.message), notes: (0, _phone_records_1.redactPhoneText)(event.technical_notes, 32000), at: event.created_at })),
