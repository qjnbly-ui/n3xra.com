...
function canonicalPayload(input) {
    ...
    if (!html && !text)
        throw new Error("HTML or text email content is required.");
    if (html.length > 1_000_000 || text.length > 1_000_000)
...
function providerFailure(message, retryable, status) {
...
async function rpc(database, functionName, body) {
...
async function sendResendEmail(input, dependencies = {}) {
    ...
    }), "communications_claim_resend_delivery");
    ...
    try {
        ...
        providerId = requiredString(responseBody.id, "Resend message ID", 200);
    }
    catch (caught) {
        const error = caught;
...
function verifyResendWebhook(input) {
    ...
    const expected = (0, node_crypto_1.createHmac)("sha256", secretBytes).update(signed).digest();
    ...
    return signatures.some((signature) => {
        ...
        if (version !== "v1" || !encoded || rest.length)
            return false;
        try {
