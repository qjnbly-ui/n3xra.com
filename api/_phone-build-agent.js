...
exports.conversationOnlyReason = conversationOnlyReason;
...
function conversationOnlyReason(text) {
    const value = text.trim();
    if (/\b(?:that(?:'s| is) not what I (?:said|asked|meant)|I (?:didn['’]?t|did not) (?:ask|say|mean)|you misunderstood|stop making changes)\b/i.test(value))
        return "correction";
    if (/^(?:(?:okay|yeah|well)[,.]?\s+)*(?:I don['’]?t understand|why\b|what\b|when\b|where\b|who\b|how\b|do you (?:know|think|understand)\b|can you (?:explain|tell|describe)\b|could you (?:explain|tell|describe)\b)/i.test(value))
        return "question";
    return "";
}
function upcomingHolidays(now) {
    ...
    const nth = (y, m, weekday, n) => 1 + (weekday - new Date(Date.UTC(y, m - 1, 1)).getUTCDay() + 7) % 7 + (n - 1) * 7;
    const events = [];
    for (const y of [year, year + 1]) {
        const add = (name, m, d) => events.push({ name, date: `${y}-${String(m).padStart(2, "0")}-${String(d).padStart(2, "0")}` });
        add("New Year’s Day", 1, 1);
        add("Martin Luther King Jr. Day", 1, nth(y, 1, 1, 3));
        add("Presidents Day", 2, nth(y, 2, 1, 3));
        add("Memorial Day", 5, 31 - (new Date(Date.UTC(y, 4, 31)).getUTCDay() + 6) % 7);
        add("Juneteenth", 6, 19);
        add("Independence Day", 7, 4);
        add("Labor Day", 9, nth(y, 9, 1, 1));
        add("Columbus Day / Indigenous Peoples’ Day", 10, nth(y, 10, 1, 2));
        add("Halloween", 10, 31);
        add("Veterans Day", 11, 11);
        add("Thanksgiving", 11, nth(y, 11, 4, 4));
        add("Christmas", 12, 25);
    }
...
const requestBuildAgent = async (messages, context, signal) => {
    ...
    const allowedTools = exports.phoneBuildTools.filter(t => (!readOnly || ["respond", "inspect_page", "get_status"].includes(t.function.name)) && (context.previewInspectionAvailable !== false || t.function.name !== "inspect_page"));
