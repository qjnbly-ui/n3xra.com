...
function bindEvents() {
    ...
    element("submission-list").addEventListener("click", (event) => { const button = event.target.closest("[data-use-submission]"); if (button)
        void useSubmission(button.dataset.useSubmission || "").catch((error) => { showStatus(error instanceof Error ? error.message : "The submission could not be opened.", true); }); });
