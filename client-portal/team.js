...
export function startOrganizationTeam({ root = document, organizationId: requestedOrganizationId = "", supabase: suppliedSupabase = null } = {}) {
    const screen = root.querySelector("#portal-status");
    const team = root.querySelector("#client-team");
    const message = root.querySelector("#team-message");
    const memberList = root.querySelector("#team-member-list");
    const inviteList = root.querySelector("#team-invite-list");
    const inviteForm = root.querySelector("#team-invite-form");
    const inviteProductAccess = root.querySelector("#team-invite-product-access");
    const limitForm = root.querySelector("#team-limit-form");
    const limitMode = limitForm?.querySelector('[name="limit-mode"]') || null;
    const limitValue = limitForm?.querySelector('[name="limit-value"]') || null;
    const limitValueWrap = limitForm?.querySelector("[data-limit-value]") || null;
    ...
    function renderMembers() {
        if (!snapshot || !memberList)
            return;
        const summary = root.querySelector("#team-member-summary");
        if (summary)
    ...
    function renderTeamLimit() {
        if (!snapshot || !limitForm || !limitMode || !limitValue || !limitValueWrap)
            return;
        ...
        limitForm.querySelectorAll("input,select,button")
            .forEach((control) => { control.disabled = !snapshot?.can_manage; });
    }
    function renderInvites() {
        if (!snapshot || !inviteList)
            return;
        const panel = root.querySelector("#team-invites-panel");
    ...
    function renderProductAccess() {
        if (!snapshot || !accessSnapshot)
            return;
        const grid = root.querySelector("#organization-product-grid");
        const head = root.querySelector("#organization-access-head");
        const body = root.querySelector("#organization-access-body");
        if (!grid || !head || !body)
            return;
    ...
    async function loadSnapshot() {
        ...
        const invitePanel = root.querySelector("#team-invite-panel");
        if (invitePanel)
        ...
        const organizationName = root.querySelector("#client-organization-name");
        const organizationStatus = root.querySelector("#client-organization-status");
        const pickerLabel = root.querySelector("#client-organization-picker-label");
        if (organizationName)
    ...
    inviteForm?.addEventListener("submit", (event) => {
        ...
        void (async () => {
            ...
            try {
                ...
                const result = await rpc("client_portal_add_or_invite_team_member", { input_organization_id: organizationId, input_recipient_email: String(values.get("email") || ""), input_recipient_name: String(values.get("name") || ""), input_role: String(values.get("role") || "viewer"), input_product_access: productAccess });
                if (result.mode === "invited" && result.id)
                    await sendInviteEmail({ id: result.id });
                inviteForm.reset();
                await loadSnapshot();
                ...
                showMessage(error instanceof Error ? error.message : "This person could not be added.", true);
            }
            finally {
                if (button) {
    ...
    limitMode?.addEventListener("change", () => {
        if (limitValueWrap)
            limitValueWrap.hidden = limitMode.value !== "custom";
    });
    limitForm?.addEventListener("submit", (event) => {
        ...
        void (async () => {
            ...
            try {
                ...
                if (limitMode?.value === "custom" && requestedLimit < 1)
                    throw new Error("Enter a team limit of at least one person.");
