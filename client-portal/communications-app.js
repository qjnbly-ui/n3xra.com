...
function setupComposer(session, workspace, organizationName, topics, topicMetrics, metrics, number, channelStates, emailDomain, senderRole, isPlatformAdmin) {
    ...
    const previewSend = document.querySelector("#communications-preview-send");
    const liveAudience = document.querySelector("#communications-live-audience");
    ...
    if (!form || !audience || !subjectField || !subject || !message || !status || !readiness || !button || !previewDialog || !previewFrame || !previewSend || !liveAudience || !testEmail || !testSend || !testStatus)
        return;
    testEmail.value = String(session.user?.email || "");
    let liveSendApproved = false;
    const selectedChannels = () => channelInputs.filter((input) => input.checked && !input.disabled).map((input) => input.value);
