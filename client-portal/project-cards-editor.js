...
function sourceLabel(resource) {
    ...
    if (resource.organization_file_id || resource.storage_path)
        return "Stored privately in Files & Assets";
...
function openResourceDialog(resource) {
    ...
    if (resource) {
        ...
        form.elements.namedItem("share_on_project_card").checked = resource.share_on_project_card;
...
async function uploadFile(file) {
    ...
    if (folderError || !folderId)
        throw new Error(folderError?.message || "Unable to prepare the private Project Cards folder.");
...
async function removeStoredFile(resource) {
    if (!resource)
        return;
...
form?.addEventListener("submit", async (event) => {
    ...
    if (type !== "text" && type !== "link" && source === "upload" && !file && !existing?.storage_path && !existing?.organization_file_id) {
        if (formStatus)
    ...
    try {
        ...
        if (existing) {
            ...
            if ((existing.organization_file_id || existing.storage_path) && (existing.organization_file_id !== organizationFileId || existing.storage_path !== storagePath))
                await removeStoredFile(existing);
        }
        else {
            const user = (await supabase.auth.getUser()).data.user;
        ...
        if (newFile)
            await removeStoredFile({ organization_file_id: newFile.id, storage_path: newFile.path });
        if (formStatus)
